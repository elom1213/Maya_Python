...
# A00340_SelectionTool - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00340_SelectionTool.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)              # .../tools/A00340_SelectionTool
...
TOOL_LABEL = "SelectTool"

_ICON_PATH = os.path.join(TOOL_ROOT, "icon", "A00340_SelectionTool.png")
ICON_NAME = _ICON_PATH if os.path.exists(_ICON_PATH) else "pythonFamily.png"

# 셸프 버튼이 실행할 명령.
# ROOT 는 JUN_All 경로여야 `import tools.A00340_SelectionTool` 가 동작한다.
SHELF_COMMAND = r'''
import sys
...
import tools.A00340_SelectionTool as A00340_SelectionTool

A00340_SelectionTool.run(True)
'''.format(
...
def install_shelf_button():
    ...
    for btn in shelf_buttons:
        ...
        if "A00340_SelectionTool.run(True)" in str(cmd):
