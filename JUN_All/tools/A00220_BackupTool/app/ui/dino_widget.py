...
class DinoWidget(QWidget):
    ...
    def __init__(self, px=3, parent=None):
        ...
        h = (_GRID_H + 8) * self._px  # 점프 여유 포함 높이
    ...
    def hop(self):
        """점프 1회 트리거(이미 점프 중이면 무시). 달리는 중에만 의미 있음."""
        if self._running and self._jump_t is None:
            self._jump_t = 0
            self._ticks_since_jump = 0
    ...
    def sizeHint(self):
        return QSize(_GRID_W * self._px + 60, (_GRID_H + 8) * self._px)
