...
# A00410_SecondaryMotion - launch entry point (Qt)
#
# FK 로 애니메이션된 컨트롤러/조인트 체인에 KawaiiPhysics 식 관성(2차 모션, 찰랑임)을
# 얹어 키 애니메이션으로 굽는 in-Maya PySide 툴.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00410_SecondaryMotion")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import.
    from tools.A00410_SecondaryMotion.app.ui.main_window import (
