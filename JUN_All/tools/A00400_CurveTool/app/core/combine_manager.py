...
#   3) 남은 임시 트랜스폼을 지운다
#   4) Keep world position 이면 CV 를 원래 **월드 위치**로 되돌린다
#      (`parent -r -s` 는 로컬 CV 값을 그대로 가져가므로 두 트랜스폼이 다르면 모양이 튄다 -
#       MEL 원본도 마찬가지다)
...
def _copy_shapes(source, target, keep_world):
    """source 커브의 쉐입을 **복사해서** target 에 붙인다. 새 쉐입 long 경로 목록."""
    src_shapes = curve_shapes(source)
    world = [_cv_world_positions(s) for s in src_shapes] if keep_world else None
...
def combine_shapes(sources, targets, keep_world=True, delete_sources=False):
    ...
    with undo_chunk():
        ...
        for s_uuid, t_uuid in uuid_pairs:
            ...
            if source == target:
                ...
                continue
            new_shapes = _copy_shapes(source, target, keep_world)
