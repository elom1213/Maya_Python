...
# A00400_CurveTool - launch entry point (Qt)
#
# 선택한 메시 엣지에 부착된 커브를 (연결 성분마다 하나씩) 만들고, 커브 방향을 축 기준으로
# 통일(Reverse Direction)하는 in-Maya PySide 툴.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00400_CurveTool")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import.
    from tools.A00400_CurveTool.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
