...
- **Chain** : 지금까지와 같다 — `Start` / `End` 쌍마다 체인을 정렬한다.
- **Root** : `Start` / `End` 자리에 **`Root` 리스트 하나**. 각 루트부터 **모든 최하위 자식까지** 정렬하고,
  루트마다 같은 줄의 `pole tgt` 하나(모자라면 마지막 것)를 `Aim axis` 로 향한다.
  갈라지는 곳에서는 **첫 번째 자식**을 조준한다(마야 `Orient Joint` 와 같은 규칙). 나머지 가지도 각자 끝까지 정렬된다.
  조인트가 아닌 루트 · 없는 루트 · 자식 없는 루트는 건너뛰고 로그에 남긴다.

**[Fix] 위치가 틀어지던 경우 — 두 모드 공통.**
X 를 자식으로 조준해도 자식이 제자리에 남는 것은 **자식이 이미 부모 X 축 위에 있을 때뿐**이었다(mayapy 실측).
분기점의 다른 가지(Root 모드), X 로 정렬되지 않은 체인에서는 자식이 옮겨졌다(예: 1.49 만큼). 이제 조인트를 돌린
직후 **움직인 자식만** 원래 월드 위치로 되돌린다. 이미 정렬된 체인은 자식이 움직이지 않으므로 결과가 예전과
**완전히 같다**(jointOrient · translate 차이 0). 잠기거나 연결된 translate 는 되돌리지 못하고 경고한다.

- 검증: mayapy 2024 34항목 — 분기 계층 두 개(각자 pole) X/보조축/위치, undo, 잘못된 입력, pole 부족,
  Chain 예전 결과 일치, 비정렬 체인 위치 보존, 잠긴 자식 경고, UI 모드 전환 · 실행.

## v03.08 (2026-09-10)
**[Change] `poleSlide` 에 배율 `0.1` — 같은 눈금으로 10배 섬세하게.**

v03.07 은 반현(양 끝 거리의 절반)을 그대로 단위로 썼다. 그러면 스핀박스 한 칸(0.1)이
**반현의 10%** 라 살짝 돌려도 타깃이 크게 움직인다. 값에 **`SLIDE_SCALE = 0.1`** 을 곱해
같은 눈금에서 **10배 섬세하게** 만들었다.

```
A' = A + n*v + (s * 0.1 / 2) * (p1 - p3)

한 칸(0.1)  = 반현의 1%   (전: 10%)
±10       = 끝까지        (전: ±1)
```

- 노드는 하나도 늘지 않는다 — `_side`(multiplyDivide) 의 Y 채널 **나눗수를 `2` 에서
  `2/SLIDE_SCALE = 20` 으로** 바꿔 반으로 나누기와 배율을 한 번에 처리한다.
- `Slide` 스핀박스의 소수점을 **네 자리**로 늘렸다(한 칸이 더 작아졌으므로).
- ★ **예전 배율로 배선된 타깃은 `Create Selected` 가 한 번 다시 짓는다.**
  그냥 두면 **같은 씬 안에서 타깃마다 감도가 달라진다** — 가장 찾기 어려운 종류의
  어깋남이다. 버전 문자열이나 이름이 아니라 **살아 있는 나눗수 값**을 읽어
  판정한다(`slide_scale_of()`) — 리네임도 레퍼런스도 타지 않는다.
- 수식·배선 구조는 v03.07 과 같다(가중치 합 1, `pointConstraint` 하나). `poleSlide = 0` 은
  예전과 완전히 같다.

### 검증 (mayapy 헤드리스 — core 51 + UI 13 항목 통과)

`s = 1` 이 반현의 **10%**, 한 칸(0.1)이 **1%** · 씬의 실제 위치도 배율을 탄다 ·
`slide_scale_of()` 가 배선된 배율을 되읽고, 나눗수를 예전 값(2.0)으로 바꾸면 **알아보고
한 번 재배선** 후 `kept` · 가중치 합 1 · v03.07 의 51항목 스위트 무회귀.

---

## v03.07 (2026-09-10)
**[Feature] `Chain > Pole Target` — `Slide` : 양 끝 사이를 오가는 두 번째 어트리뷰트.**

`Create` · `Create Selected` 로 세팅한 오브젝트에 실수 어트리뷰트 **`poleSlide`** 가 하나
더 붙는다. **양수면 리스트의 첫 오브젝트(A)와, 음수면 마지막 오브젝트(C)와
가까워진다.** `poleDistance` 와 마찬가지로 씬에서 실시간으로 돌리면 바로 움직인다.

```
A' = A + n*v + (s/2)*(p1 - p3)          s = poleSlide
s = +1 -> 기준점이 p1 (첫 오브젝트)까지,  s = -1 -> p3 (마지막)까지
```

- 단위는 **반현(양 끝 거리의 절반)** 이라 `±1` 이 "끝까지" 다 — 리그 크기가 달라져도
  값을 다시 잡을 필요가 없다. `Fixed distance` 켜고 끄는 것과 무관하게 둘 다 돌아간다.
- ★ **이것도 가중치 합이 1 이라 여전히 `pointConstraint` 하나**다 —
  `w0 = (1-n)/2 + s/2`, `w1 = n`, `w2 = (1-n)/2 - s/2`. `s/2` 는 **새 노드 없이**
  `_side`(multiplyDivide) 의 **Y 채널**로 계산해서, 늘어난 노드는 가중치 합성용
  `plusMinusAverage` **둘**뿐이다.
- ★ **슬라이드를 `n` 에 섞지 않는 것이 핵심.** 기준점 `A` 를 먼저 움직인 뒤
  거기서 `n*v` 를 가면 가중치가 `(1-n)*s/2` 가 되어 **`n > 1` 에서 부호가 뒤집힌다.**
  폴 타깃은 보통 `n > 1` 이므로 "양수를 넣었는데 C 쪽으로 간다" 가 된다.
  슬라이드는 **마지막에 더하는 평행이동**이어야 한다.
- `Update Selected` 가 `Distance` 와 `Slide` 를 **함께** 넣는다.
- **예전(v03.06 이하)에 배선해 둔 타깃은 `Create Selected` 를 한 번 누를 때 다시 짓는다**
  (`poleSlide` 가 없으면 `kept` 로 둘 수 없다 — 그대로 두면 버튼을 눌러도 슬라이드가
  생기지 않는다). 그 뒤로는 `kept` 다.
- `Fixed distance` 와 같이 쓰면 가운데 오브젝트까지의 거리가 더 이상 정확히
  `Distance` 가 아니다(직각삼각형으로 조금 늘어난다) — 의도된 동작이다.
- `A00130_ControlRig_V02` 는 `slide` 를 안 넘기므로 **놓이는 자리가 예전과 같다**(기본 0).
  타깃에 `poleSlide` 가 생기므로 필요하면 그쪽에서도 돌릴 수 있다.

### 검증 (mayapy 헤드리스 — core 41 + UI 11 항목 통과)

슬라이드를 올리면 A 와의 거리가 줄고 C 와의 거리가 늘며(반대도 마찬가지) ·
**씬의 실제 위치 == 수식**(두 모드 · 6개 값) · **`n > 1` 에서도 부호가 안 뒤집힌다** ·
가중치 합이 정확히 1 · `bake` 가 늘어난 노드까지 전부 치운다 · 예전 타깃은 한 번만
재배선되고 다음부터 `kept` · `slide=0` 은 예전과 바이트 단위로 같다(무회귀).

---

## v03.06 (2026-09-10)
**[Feature] `fixed_from_multiple()` — 배수로 잡아 둔 규칙을 거리 고정 배선으로 옮기는 환산.**

`A' = A + n*v` 와 `A' = p2 + d*v/|v|` 는 `d = (n-1)*|v|` 에서 **같은 점**이다(부호까지).
배수로 적어 둔 규칙 파일을 거리 고정 배선으로 넘길 때 값을 그대로 주면 뜻이 달라져 타깃이
가운데 오브젝트에 달라붙는다 — 이 환산을 거치면 **놓이는 자리가 예전과 똑같고** 그 뒤로만
거리가 유지된다.

`A00130_ControlRig_V02` 의 템플릿 폴 타깃 4개가 이걸 쓴다 (그 툴 v02.16).

---

## v03.05 (2026-09-10)
**[Feature] `Chain > Pole Target` — `Fixed distance` : 체인이 굽어도 가운데 오브젝트와의 거리가 그대로.**

예전 배선에서 타깃과 가운데 오브젝트의 거리는

```
|A' - p2| = |n - 1| * |v|          (v = p2 - (p1+p3)/2)
```

라 **굽은 정도에 딸려 변했다.** 팔을 펴면 타깃이 팔꿈치로 빨려 들어오고 접으면 멀어진다 —
폴 벡터 타깃으로 쓰기에는 거슬리는 쪽이다. `Fixed distance`(기본 ON)를 켜면

```
A' = p2 + d * v / |v|              <- 언제나 d 만큼 떨어져 있다
```

가 되어 **A · B · C 를 어떻게 움직여도 거리가 `poleDistance` 값 그대로**다.

**★ 정규화를 하면서도 컨스트레인트 구성은 그대로다.** 위 식은 `A' = A + n*v` 에서

```
n = 1 + d / |v|
```

과 **같은 식**이다. 즉 `pointConstraint` 는 손대지 않고 거기에 먹이는 `n` 만 상수에서
**계산된 값**으로 바꾸면 된다 — 월드 공간과 부모 공간 변환은 여전히 컨스트레인트가 공짜로
해 준다. `|v|` 는 `decomposeMatrix` 둘 + `plusMinusAverage`(average) + `distanceBetween`
넷으로 잰다.

**★ `multiplyDivide` 의 0 나누기는 0 도 NaN 도 아니고 `100000` 이다** (실측, 경고만 낸다).
더 위험한 것은 `|v|` 가 0 이 아니라 **아주 작을 때**다 — `n` 이 1e9 쯤으로 뛰면 가중 평균이
큰 수끼리의 뺄셈이 되어 자릿수가 통째로 날아간다. 그래서 나누기 앞에 `clamp` 로 `|v|` 의
하한을 `STRAIGHT_EPS` 로 잡았다. 일직선 체인에서 타깃은 **가운데 오브젝트 자리에 머문다**(폭발 없음).

**★ 모드가 다르면 `kept` 가 아니라 다시 짓는다.** 체인이 같아도 배선 모드가 요청과 다르면
재배선한다 — 안 그러면 체크박스를 켜고 `Create Selected` 를 눌러도 아무 일도 안 일어난
것처럼 보인다. 이때 컨스트레인트만 지우면 계산 노드가 떠돌이로 남으므로, 가중치에서
거슬러 올라가 **우리가 만든 유틸리티 노드만** 골라 함께 지운다(`helper_nodes()`).
`Bake Selected` 도 같은 경로를 쓴다 — 계산 노드가 여러 단이라 한 단만 지우면 남는다.

`Fixed distance` 를 끄면 v03.04 까지의 배수 동작 그대로다. **`A00130_ControlRig_V02` 가 쓰는
`ensure()` 의 기본값은 배수**라 그쪽 템플릿 폴 타깃은 이번 변경의 영향을 받지 않는다.

검증 **25항목** (mayapy 2024 headless) — 네 가지 포즈에서 거리 유지 · 밀어내는 방향 보존 ·
`poleDistance` 가 거리에 1:1(음수는 반대쪽) · 일직선 안전 · `Create Selected` 동일 ·
undo 한 스텝 · bake/재배선 뒤 떠돌이 노드 0 · 모드 전환 재배선 · `A00130` 경로 무변경 ·
`Check` 미리보기 == 실제 결과. v03.04 스위트 22항목도 그대로 통과.

---

## v03.04 (2026-09-10)
**[Feature] `Chain > Pole Target` — `Create Selected` : 이미 있는 오브젝트를 폴 타깃으로.**

`Create` 와 **같은 배선**을 새 노드가 아니라 **씬에서 고른 오브젝트**에 건다
(`pole_target_manager.create_on()`). 리스트에 세 오브젝트를 담아 두고 대상들을 고른 뒤
누르면, 그 오브젝트들이 그대로 `A' = A + n*v` 에 붙는다. 이름 · 부모 · 셰이프 · 이미
걸려 있던 다른 연결은 **하나도 건드리지 않는다.**

이미 만들어 둔 컨트롤러를 폴 타깃으로 쓰고 싶을 때가 새 로케이터를 만드는 경우보다
흔하다. 예전에는 `Create` 로 만든 뒤 옛 오브젝트를 지우고 이름을 옮겨야 했다.

**★ 체인 멤버는 건너뛴다.** 리스트에 담긴 세 오브젝트 자신을 고른 채 누르면 **자기 자신을
타깃으로 삼는 컨스트레인트**가 되어 순환이 된다. 마야는 사이클 경고만 내고 씬은 망가진
채로 남으므로 걸기 전에 막고 로그로 알린다.

**★ 이미 배선된 타깃의 거리는 덮어쓰지 않는다.** `poleDistance` 는 실시간으로 맞춰 두라고
만든 값이다 — 두 번째로 눌러도 `kept` 로 두고, 바꾸려면 `Update Selected` 를 쓴다.
`ensure()` 가 `A00130` 에서 쓰는 규칙과 같다.

남이 건 컨스트레인트가 이미 있는 오브젝트도 **조용히 지우지 않고** 건너뛴다.
여러 개를 골라도 **전체가 undo 한 스텝**이다.

UI 는 남는 폭을 통째로 쓰던 `Create` 버튼의 **가로를 반으로** 줄여 그 옆에
`Create Selected` 를 세웠다 — `Check` / `Create` / `Create Selected` 가 한 줄이라
버튼 줄이 늘지 않는다.

검증 **22항목** (mayapy 2024 headless) — 배선 · 위치가 `A'` 와 일치 · 체인을 움직이면
따라옴 · undo 한 스텝 · 여러 개 동시 · 체인 멤버 가드 · 리스트가 3개가 아닐 때 ·
두 번째 실행 `kept` · 남의 컨스트레인트 보존 · 버튼 배치.
