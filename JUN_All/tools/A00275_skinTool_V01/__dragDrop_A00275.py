...
# A00275_skinTool_V01 - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00275_skinTool_V01.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)              # .../tools/A00275_skinTool_V01
...
_ICON_PATH = os.path.join(TOOL_ROOT, "icon", "A00275_skinTool_V01.png")
...
import tools.A00275_skinTool_V01 as A00275_skinTool_V01

A00275_skinTool_V01.run(True)
'''.format(
...
def install_shelf_button():
    ...
    for btn in shelf_buttons:
        ...
        if "A00275_skinTool_V01.run(True)" in str(cmd):
