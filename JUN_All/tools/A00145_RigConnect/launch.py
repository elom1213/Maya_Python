...
# A00145_RigConnect - launch entry point (Qt)
#
# MEL ConnectionTool V04.02 (Constrain / Connect / List Connected) +
# A00140 ConnectClosest (Connect Closest) 병합 툴.
# UI 는 PySide(QTabWidget), 로직은 maya.cmds 포팅.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00145_RigConnect")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import.
    from tools.A00145_RigConnect.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
