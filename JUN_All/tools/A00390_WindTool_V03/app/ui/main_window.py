...
from tools.A00390_WindTool_V03.app.core import wave_lite_manager as lite_mgr


WINDOW_OBJECT_NAME = "JUN_A00390_WindTool_V03_window"
...
class MainWindow(QWidget):
    ...
    def build_ui(self):
        ...
        pb = self.tabs.addTab(self._build_playback_tab(), "Playback")
        self.tabs.setTabToolTip(
            pb, "Playback speed: rebuild the phase clock after keying windSpeed,@"
                "and mute / solo chains so only the ones you are looking at move.@"
                "Works on every tab's drivers."
                .replace("@", chr(10)))
    ...
    # ==============================================================
    # Playback  (세 탭 공통 - 재생 성능)
    # ==============================================================

    def _build_playback_tab(self):
        """위상 시계 다시 배선 + 체인 mute/solo. 세 탭이 만든 드라이버 전부에 걸린다.

        V02 는 드라이버마다 표현식을 하나씩 붙였고, 그 표현식이 매 프레임
        `getAttr -time` 을 시작 프레임부터 현재 프레임까지 반복 호출했다. 재생 비용의
        99.8% 가 거기였다(30체인 58.7ms). V03 은 상수 속도를 노드 3개로 처리하고,
        windSpeed 에 키가 걸린 것만 **씬에 하나뿐인 통합 표현식**이 맡는다.
        """
        page = QWidget()
        pb = QVBoxLayout(page)

        note = QLabel(
            "These act on the wind drivers of ALL tabs (Sine / Chain Wave /@"
            "Chain Wave Lite), not just the tab you are on.".replace("@", chr(10)))
        note.setWordWrap(True)
        pb.addWidget(note)

        # ---- Phase clock
        phase_box = QGroupBox("Phase clock (windPhaseTime)")
        phase = QVBoxLayout(phase_box)

        phase_note = QLabel(
            "A constant windSpeed is driven by 3 nodes - nothing to rebuild.@"
            "Press Rebuild only after you ADD or REMOVE keys on windSpeed, so the@"
            "driver switches between the node path and the live integral."
            .replace("@", chr(10)))
        phase_note.setWordWrap(True)
        phase.addWidget(phase_note)

        self.btn_rebuild_phase = QPushButton("Rebuild Phase")
        self.btn_rebuild_phase.setMinimumHeight(32)
        self.btn_rebuild_phase.setToolTip(
            "Re-wire windPhaseTime on every wind driver in the scene.@@"
            "Constant windSpeed  ->  3 nodes (time -> x speed -> + start).@"
            "Keyed windSpeed     ->  a live integral inside ONE shared@"
            "                        'windPhaseEngine' expression.@@"
            "The keyed path reads the KEY DATA of the curve, so editing those keys@"
            "updates the wave immediately - you do not need to press this again.@"
            "Press it when you first key windSpeed, or when you delete those keys."
            .replace("@", chr(10)))
        self.btn_rebuild_phase.clicked.connect(self.on_rebuild_phase)
        phase.addWidget(self.btn_rebuild_phase)

        self.btn_bake_phase = QPushButton("Bake Phase to Keys (V02-exact)")
        self.btn_bake_phase.setToolTip(
            "Bake windPhaseTime into an animCurve over the playback range using the@"
            "same per-FRAME trapezoid integration V02 used, so the values match V02@"
            "exactly even on spline tangents.@@"
            "Use this only when you must reproduce a V02 scene number-for-number.@"
            "It is NOT live - bake again after editing windSpeed.@@"
            "The live path integrates per KEY INTERVAL instead, which ignores the@"
            "curvature between keys (identical on linear tangents, up to ~21%@"
            "different on spline ones)."
            .replace("@", chr(10)))
        self.btn_bake_phase.clicked.connect(self.on_bake_phase)
        phase.addWidget(self.btn_bake_phase)
        pb.addWidget(phase_box)

        # ---- Mute / Solo
        mute_box = QGroupBox("Mute chains")
        mute = QVBoxLayout(mute_box)

        mute_note = QLabel(
            "Muted chains hold their REST pose and stop being evaluated.@"
            "Their debug curves are hidden too. windEnvelope = 0 does NOT do this -@"
            "it only multiplies the swing by zero while everything still evaluates."
            .replace("@", chr(10)))
        mute_note.setWordWrap(True)
        mute.addWidget(mute_note)

        row = QHBoxLayout()
        self.btn_solo = QPushButton("Solo Selected")
        self.btn_solo.setToolTip(
            "Keep the SELECTED driver locator(s) playing and mute every other chain.@@"
            "Select the driver locators (the ones holding windSwingAngle etc.),@"
            "not the joints.")
        self.btn_solo.clicked.connect(self.on_solo)
        row.addWidget(self.btn_solo)

        self.btn_unmute = QPushButton("Unmute All")
        self.btn_unmute.setToolTip("Put every chain back to normal evaluation.")
        self.btn_unmute.clicked.connect(self.on_unmute_all)
        row.addWidget(self.btn_unmute)

        self.btn_mute_all = QPushButton("Mute All")
        self.btn_mute_all.setToolTip(
            "Mute every chain - the whole rig stands at rest and stops evaluating.")
        self.btn_mute_all.clicked.connect(self.on_mute_all)
        row.addWidget(self.btn_mute_all)
        mute.addLayout(row)

        status_row = QHBoxLayout()
        self.lbl_mute_status = QLabel("-")
        status_row.addWidget(self.lbl_mute_status, 1)
        self.btn_mute_refresh = QPushButton("Refresh")
        self.btn_mute_refresh.setToolTip("Re-count the drivers in the scene.")
        self.btn_mute_refresh.clicked.connect(self._refresh_mute_status)
        status_row.addWidget(self.btn_mute_refresh)
        mute.addLayout(status_row)
        pb.addWidget(mute_box)

        pb.addStretch(1)
        self._refresh_mute_status()
        return page

    def _refresh_mute_status(self, *args):
        try:
            total = len(wind_mgr.all_wind_drivers())
            muted = len(wind_mgr.muted_drivers())
        except Exception:                                 # noqa: BLE001
            self.lbl_mute_status.setText("-")
            return
        if not total:
            self.lbl_mute_status.setText("No wind driver in the scene.")
        else:
            self.lbl_mute_status.setText(
                "{0} driver(s): {1} playing, {2} muted.".format(
                    total, total - muted, muted))

    def on_rebuild_phase(self):
        with undo_chunk():
            try:
                _n, msg = wind_mgr.rewire_scene_phase()
            except Exception as exc:                      # noqa: BLE001
                self.log("[Error] {0}".format(exc), warn=True)
                return
        self.log(msg, warn=msg.startswith("[Warning]"))

    def on_bake_phase(self):
        rng = self.lite_range.values()
        start, end = rng if rng else (None, None)
        with undo_chunk():
            try:
                _n, msg = wind_mgr.bake_scene_phase(start, end)
            except Exception as exc:                      # noqa: BLE001
                self.log("[Error] {0}".format(exc), warn=True)
                return
        self.log(msg, warn=msg.startswith("[Warning]"))
    ...
    def on_solo(self):
        ...
        if not sel:
            self.log("Select the driver locator(s) to keep playing.", warn=True)
            return
        with undo_chunk():
            try:
                _k, _m, msg = wind_mgr.solo_chains(sel)
            except Exception as exc:                      # noqa: BLE001
                self.log("[Error] {0}".format(exc), warn=True)
                return
        self._refresh_mute_status()
    ...
    def on_mute_all(self):
        self._set_mute_all(True)

    def on_unmute_all(self):
        self._set_mute_all(False)

    def _set_mute_all(self, mute):
        with undo_chunk():
            try:
                _d, _n, msg = wind_mgr.set_mute_all(mute)
            except Exception as exc:                      # noqa: BLE001
                self.log("[Error] {0}".format(exc), warn=True)
                return
        self._refresh_mute_status()
