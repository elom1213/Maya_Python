# -*- coding: utf-8 -*-
# A00390_WindTool_V03 core - Chain Wave **Lite** : 커브·ikHandle·프록시 없이
...
from .wind_manager import (
    ...
    _sine_lut, _leaf, _safe, follow_root_position, wire_phase,
)
...
def _make_lite_driver(name, swing, wavelength, period, speed, ramp, phase_offset=0.0,
    ...
    # 위상 시계 배선은 호출부가 wire_phase() 로 한다(V03). 상수 속도면 노드 3개,
    # windSpeed 에 키가 걸려 있으면 씬에 하나뿐인 통합 표현식이 맡는다.
...
def build_wave_lite(joints, mode=MODE_ROOT, axis="Y", swing=20.0, wavelength=10.0,
    ...
    # ---- 위상 시계 배선 (V03). **그룹핑이 끝난 뒤**에 해야 드라이버 풀패스가 확정된다.
    # 상수 속도면 드라이버당 노드 3개, windSpeed 에 키가 걸려 있으면 씬에 하나뿐인
    # 통합 표현식이 맡는다. V02 는 드라이버마다 표현식을 붙였고 그게 재생 비용의
    # 99.8% 였다 - `wind_manager` 의 '위상 시계 엔진' 주석 참고.
    phase_made, phase_const, phase_keyed, _phase_map = wire_phase(drivers)
    made_all += phase_made
    ...
    msg += " Phase: {0} constant (node path){1}.".format(
        phase_const,
        ", {0} keyed (live integral)".format(phase_keyed) if phase_keyed else "")
