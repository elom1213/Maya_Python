# -*- coding: utf-8 -*-
# A00390_WindTool_V03 core - Chain Wave : 조인트 체인이 **회전만으로** 싸인 파형을 따라가게 한다.
#                                     (maya.cmds, UI 비의존)
#
# ## 왜 따로 만들었나
#
...
def _make_wave_driver(name, amplitude, wavelength, period, speed, ramp,
    ...
    # 위상 시계 배선은 호출부가 wire_phase() 로 한다(V03). 아래 참고:
    #   wind_manager 의 '위상 시계(windPhaseTime) 엔진' 주석
...
def build_wave(joints, mode=MODE_ROOT, axis="Y", amplitude=1.0, wavelength=10.0,
    ...
    # ---- 위상 시계 배선 (V03) - wind_manager 의 '위상 시계 엔진' 주석 참고.
