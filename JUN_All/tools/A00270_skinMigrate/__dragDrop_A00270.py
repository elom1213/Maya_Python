...
# A00270_skinMigrate - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00270_skinMigrate.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)              # .../tools/A00270_skinMigrate
...
TOOL_LABEL = "SkinMigrate"

_ICON_PATH = os.path.join(TOOL_ROOT, "icon", "A00270_skinMigrate.png")
...
import tools.A00270_skinMigrate as A00270_skinMigrate

A00270_skinMigrate.run(True)
'''.format(
...
def install_shelf_button():
    ...
    for btn in shelf_buttons:
        ...
        if "A00270_skinMigrate.run(True)" in str(cmd):
