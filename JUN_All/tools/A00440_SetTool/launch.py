...
# A00440_SetTool - launch entry point (Qt)
#
# 컴포넌트를 원소로 갖는 마야 오브젝트 세트들끼리 집합 연산을 하는 in-Maya PySide 툴.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00440_SetTool")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import.
    from tools.A00440_SetTool.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
