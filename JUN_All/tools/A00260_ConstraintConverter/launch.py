...
# A00260_ConstraintConverter - launch entry point (Qt in Maya)
...
def run(reload_module=True):
    """
    UI 실행 진입점.
    reload_module=True 이고 DEV_MODE 면 자기 자신 + Framework 를 리로드한 뒤 실행한다.
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00260_ConstraintConverter")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import
    from tools.A00260_ConstraintConverter.app.ui.main_window import (
        MainWindow, WINDOW_OBJECT_NAME,
    )
    from Framework.qt.qt import QApplication

    # objectName 으로 떠 있는 기존 창을 모두 찾아 닫는다 (창이 쌓이는 것 방지).
