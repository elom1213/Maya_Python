...
# A00430_DemBone - launch entry point (Qt)
#
# 애니메이션 메시 시퀀스(알렘빅 캐시)와 조인트로부터 스킨 웨이트 / 본 애니메이션을 푸는
# in-Maya PySide 툴. EA Dem Bones(BSD 3-Clause) 알고리즘의 마야 이식.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00430_DemBone")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import.
    from tools.A00430_DemBone.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
