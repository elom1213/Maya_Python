...
# A00370_ToolLauncher - launch entry point (Qt, in-Maya)
#
# 사용자가 만든 버튼을 누르면 지정한 JUN 툴이 팝업되는 숏컷 런처.
# 셸프 버튼은 tools.A00370_ToolLauncher.run(True) 를 호출한다.
...
def run(reload_module=True):
    """
    UI 실행 진입점.
    reload_module=True 이고 DEV_MODE 면 자기 패키지 + Framework 를 리로드한 뒤 실행한다.
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00370_ToolLauncher")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import (리로드 순서 문제 회피)
    from tools.A00370_ToolLauncher.app.ui.main_window import (
    ...
    ThemeManager.load_theme_to_widget(window_instance, "yellow_light")
