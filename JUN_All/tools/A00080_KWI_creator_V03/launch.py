...
# A00080_KWI_creator_V03 - launch entry point (Qt, in Maya)
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00080_KWI_creator_V03")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import
    from tools.A00080_KWI_creator_V03.app.ui.main_window import (
        MainWindow,
        WINDOW_OBJECT_NAME,
    )
    from Framework.qt.qt import QApplication

    # objectName 으로 떠 있는 기존 창을 모두 닫는다 (창이 여러 개 쌓이는 문제 방지)
    ...
    ThemeManager.load_theme_to_widget(window_instance, "purple_dark")
