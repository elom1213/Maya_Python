...
# A00310_SearchTool - launch entry point (Qt)
#
# 레거시 JUN_PY_SelectionTool + JUN_PY_SearchTool 을 하나의 탭 UI 로 통합한 툴.
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        ...
        reload_for_tool("tools.A00310_SearchTool")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import (리로드 순서 문제 회피)
    from tools.A00310_SearchTool.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
