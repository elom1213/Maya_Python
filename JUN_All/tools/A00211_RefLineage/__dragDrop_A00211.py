...
# A00211_RefLineage - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00211_RefLineage.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)                       # .../tools/A00211_RefLineage
...
TOOL_LABEL = "RefLineage"

_ICON_PATH = os.path.join(TOOL_ROOT, "icon", "A00211_RefLineage.png")
ICON_NAME = _ICON_PATH if os.path.exists(_ICON_PATH) else "pythonFamily.png"

# 셸프 버튼이 실행할 명령. ROOT 는 JUN_All 경로여야 import tools.A00211_RefLineage 가 동작.
SHELF_COMMAND = r'''
import sys
...
import tools.A00211_RefLineage as A00211_RefLineage

A00211_RefLineage.run(True)
'''.format(
...
def install_shelf_button():
    ...
    for btn in shelf_buttons:
        ...
        if "A00211_RefLineage.run(True)" in str(cmd):
...
def onMayaDroppedPythonFile(*args):
    ...
    try:
        ...
        # 베이스네임으로 import 되어 sys.modules 에 캐시되므로, 다른 툴의 드롭 파일과
        # 충돌하지 않도록(또 캐시된 이전 모듈 재실행 방지) 자기 자신을 캐시에서 제거한다.
        import sys
