...
# A00250_SceneMemo - launch entry point (Qt)
...
def run(reload_module=True):
    ...
    if reload_module and getattr(jun_config, "DEV_MODE", False):
        # 자기 자신 + Framework 만 reload (다른 툴 창에 영향 주지 않도록).
        from dev.reloader_v02 import reload_for_tool
        reload_for_tool("tools.A00250_SceneMemo")

    # 리로드 후 갱신된 클래스를 잡기 위해 지역 import (리로드 순서 문제 회피)
    from tools.A00250_SceneMemo.app.ui.main_window import MainWindow, WINDOW_OBJECT_NAME
    from Framework.qt.qt import QApplication

    # 기존에 떠 있는 같은 창을 objectName 으로 찾아 모두 닫는다 (창이 쌓이는 문제 방지).
