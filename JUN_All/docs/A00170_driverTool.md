...
1. `AttachCrv > Edge Loop` 로 만든 **위 입술 널**(또는 그 조인트)을 `Upper`, **아래 입술**을
   `Lower` 리스트에 담는다. 조인트를 담아도 그 조인트를 끄는 널을 자동으로 찾아간다.
2. `Controller` 에 어트리뷰트를 올릴 컨트롤을 지정(**Get**).
3. **Corner Axis**(기본 World X)와 **sealR starts at**(Min / Max 끝)으로 지퍼 방향을 정하고
   **Preview Pairing** 으로 짝을 확인한다(씬은 건드리지 않는다).
4. **Build Seal**.
...
| `sealBias` | 0~1 | 만나는 높이(0=아랫입술 자리, 0.5=중간, 1=윗입술 자리) |
| `sealBand` | 0.02~1 | 지퍼가 부드러운 정도(작으면 하나씩 딱딱, 크면 여러 개가 함께) |

`sealBias` / `sealBand` 는 빌드 후에도 **라이브로** 조절된다(노드로 물려 있다).

#### 원리 (요약)

- 조인트 쌍의 **커브 위 두 점을 `sealBias` 로 섞은 점**이 다물리는 자리다 — 커브를 움직이면
  그 자리도 따라간다.
...
- v1 은 **위치만** 블렌드한다(회전은 커브 구동 그대로). 살이 눌리는 볼륨 변화는 코렉티브
  블렌드셰이프(`A00280` / `A00290`) 영역이다.
