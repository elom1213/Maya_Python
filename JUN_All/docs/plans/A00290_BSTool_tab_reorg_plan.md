...
- **상태**: **적용 완료 — `A00290_BSTool_V02` v02.00 (2026-09-16).** 11장 1~7단계 끝,
  ...
  **6-1 은 A(`Extract`)**, **6-5 는 A(감싼다) + "스크롤이 안 생기게 창을 키운다"**,
  ...
  6-5 실측(최종): 창은 **620 x 1000** 이고 **일곱 페이지 어디에도 가로 스크롤이 없다.**
  처음엔 1400 x 1158 로 잡았는데 그건 **테마 없이 잰 값**이라 과했다 — 테마 qss 의
  `font-size: 12px` 로 최소 폭이 1092 → 809px 로 준다(**창 크기는 테마를 입히고 재야 한다**).
  남은 벽은 **한 줄에 나란히 둔 버튼·라디오·라벨**이었다. 그 줄의 최소 폭이 그대로 창 최소 폭이
  되고 좌우 스플리터 안에서는 두 배가 된다. 자리만 내려 `Mix Targets` 1092 → **573px**,
  `Naming` 792 → **498px**(기능·버튼·라벨은 그대로).
  ※ **세로는 반대로 움직인다** — 좁히면 글이 접혀 필요한 세로가 늘어난다(Shape Editor 918).
  세로까지 없애려면 약 1195px 이 필요한데 1080p 화면에 안 들어가므로, 화면에 들어가는 높이를
  택하고 세 페이지는 세로 스크롤이 받게 뒀다.
