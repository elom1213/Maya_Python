...
- **작성일**: 2026-09-16
- **대상**: `tools/A00290_BSTool/app/ui/main_window.py` (UI 전용 — `app/core/*` 는 손대지 않는다)
- **상태**: **계획 — 착수 전.** 6장의 결정 두 가지(6-1 라벨 · 6-5 스크롤)를 정한 뒤 시작한다.
- **버전**: v01.23 → **v01.24**
...
**가르지 않는다.** UI 묶기뿐이라 되돌릴 일이 생겨도 커밋 하나를 되돌리면 된다.
제자리에서 v01.24 로 올린다.
...
7. 문서 갱신 — `docs/A00290_BSTool.md` 의 절 제목(`탭 1` ~ `탭 6`)을 새 구조로,
   `CHANGELOG.md` v01.24(5장 매핑 표 포함), `WORKLOG.md`,
