...
| 항목 | 결정 |
|------|------|
| 파일 구조 | ~~**단일 누적 파일**~~ → **월 롤링** (2026-08-25 개정, 아래 참고) · 최신 날짜가 위 |
...
## 개정 이력 — 2026-08-25 월 롤링 도입

`단일 누적 파일` 결정의 전제가 2.4개월 만에 무너졌다.

- 실측: **4,742줄 / 497KB / 44 작업일** → 하루 **110줄 · 11.6KB**. 연 환산 **~24,000줄 / 2.5MB**.
- 결정적 비용은 스크롤이 아니라 **컨텍스트** — 308,724자 ≈ **15만~24만 토큰**이라
  세션에서 파일을 통째로 열 수 없었다.
- 그래서 **루트 `WORKLOG.md` = 현재 월**, 지난 달은 **`JUN_All/docs/worklog/YYYY-MM.md`** 로 내린다.
  **루트 경로는 고정**(이 경로를 가리키는 곳이 60군데).
- 이 계획서가 예견한 `worklog/` 폴더 대안(아래 "산출물 위치")이 실제로 채택된 셈이다.
  다만 루트 파일을 폴더 안으로 옮기지는 않았다 — **링크 보존이 우선**이라 폴더는 보관용으로만 쓴다.
- 규칙 · 절차 전문: [`../worklog/README.md`](../worklog/README.md)
