...
- **상태**: **적용 완료 — v01.15 (2026-08-24).** 11장 1~6단계 끝, 7단계(사용자 Maya 확인) 대기.
  9-3 실측: 하위 탭 바 폭 최대 246px(창 620 에 여유) · 가장 긴 페이지는 `Expand Bind` 약 850px
  → 창 기본 높이 680 → **710**, 나머지는 스크롤이 받는다.
  9-4 실측: `find_editing_in_scene()` 은 메시 5,000개 씬에서 **205ms**. 8장 권장안대로
  `Edit > Edit Mesh` 가 보이고 대상이 비어 있을 때만 돌므로 **그대로 둔다**
  (`cmds.ls("*.<attr>")` 로 21ms 까지 줄지만 중복·네임스페이스 결과가 달라 별도 작업으로 미룬다).
- **버전**: v01.14 → **v01.15**
