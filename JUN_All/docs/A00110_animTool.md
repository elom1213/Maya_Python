...
- **Ctrl+Z**: 조작이 **멎은 시점**(약 350ms, 슬라이더를 놓거나 스핀박스 편집을 끝내면 즉시)에 결과가
...
- **Apply Stagger Offset**: 현재 값을 **못박고** 세션을 닫는다(조작이 멎으면 이미 기록되므로 확인용).
  확정 후 슬라이더/스핀박스는 0 으로 초기화된다(같은 값이 두 번 적용되는 사고 방지).
...
3. **Offset per Item** 슬라이더를 끌거나 스핀박스를 돌린다 → **즉시 씬에 반영**되어 결과를 보면서 정할 수 있다.
   마음에 안 들면 값을 다시 바꾸거나, **Ctrl+Z**(조작 직전으로) 또는 **Reset**(원위치).
4. **Apply Stagger Offset** → 값을 못박고 세션 종료. 슬라이더/스핀박스는 0 으로 초기화된다.
