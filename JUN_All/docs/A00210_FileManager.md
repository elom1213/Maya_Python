...
| **Store Repo** | (git 모드) 기록(JSON)·썸네일(PNG)을 모아두는 **중앙 git 데이터 리포**의 로컬 clone(예: `JUN_FileManager_data`). |
| **Shared Folder** | (local 모드) 기록·썸네일을 직접 읽고 쓰는 **공유/NAS 폴더**. git 없이 폴더 자체를 데이터 스토어로 쓴다. |
...
│ Project Root [............] [Browse]                   │
│ Store Repo   [............] [Browse]   ← git 모드       │
│ Shared Folder[............] [Browse]   ← local 모드     │
│ Scan Dir     [............] [Browse]                   │
...
1. **설정**: `Source Mode` 를 고른 뒤 `Project Root`·`Author` 와 모드별 경로를 채우고 **Save Settings**.
   - **Remote (Git)**: `Store Repo`, `Remote`/`Branch`(필요 시 `Remote URL`)를 채운다.
   - **Local (Shared / NAS)**: `Shared Folder` 에 공유/NAS 폴더만 지정하면 된다(git 입력·Pull/Push 비활성).
     자세한 흐름은 **5-C** 참고.
...
## 4-C. Path Structure 탭 — 폴더 구조 템플릿 (v01.01, 선택 기록 v01.07, 깊이·선택 재생성 v01.24)
...
│ └──────────────────────────────────────────────────┘ │
│ Name [................]                                │
│ Capture Depth [ 1 ]                       [Capture][Save]│
└────────────────────────────────────────────────────────┘
┌ Saved Structures ─────────────────────────────────────┐
│ (저장된 구조 목록)   [Refresh][Recreate][Delete]      │
│ Preview       Depth[All] [ ] Show files   [Expand]    │
│ ┌──────────────────────────────────────────────────┐ │
│ │ [v] 00_asset                                       │ │  ← 폴더=체크박스, 파일=표시만
...
- **Depth**: Preview 에 보이는 **동시에 Recreate 가 생성하는** 폴더 깊이 제한(0 = All).
- **Show files**(기본 **끔**): 켜면 로컬 파일시스템(현재 base 폴더)에서 실제 **파일**도 함께 보여준다.
  파일은 **확인용**일 뿐 Recreate 대상이 아니다(폴더만 생성).
- **Expand**: 같은 트리를 **큰 창**으로 연다. 큰 창에서 바꾼 체크 상태는 탭에 그대로 반영된다.
- 각 **폴더 항목의 체크박스**: 체크를 **해제하면 Recreate 에서 제외**된다(루트=base 는 항상 생성).
  단, 체크된 하위 폴더가 필요로 하는 **상위 폴더는 자동 생성**된다.
- **다중 선택 일괄 토글**: 트리에서 **Shift/Ctrl** 로 여러 행을 선택한 뒤 선택된 항목 중 하나의
  체크박스를 누르면 **선택된 폴더 전체가 동시에 체크/해제**된다.

- **Recreate**(v01.24): 저장된 구조를 선택해 현재 `Project Root` 아래에, **Depth 이내 + 체크된 폴더만**
  생성한다(이미 있으면 건너뜀). 즉 원하는 **깊이·경로만** 골라 재생성할 수 있다.

**사용 흐름**: Base 지정 → (Scan) → 기록할 폴더 체크(또는 **All**) → **Capture Depth** 지정 →
**Capture** → **Name** 입력 → **Save** → **Push**. 받는 PC 에서 같은 구조 선택 →
(**Depth** 조절·필요 없는 폴더 체크 해제) → **Recreate**.
