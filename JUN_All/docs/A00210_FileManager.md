...
## 4-C. Path Structure 탭 — 폴더 구조 템플릿 (v01.01, 선택 기록 v01.07, 깊이·선택 재생성 v01.24, Rename·Recreate To v01.28)

베이스 폴더의 **하위 폴더 구조**를 JSON 으로 저장(`<store_dir>/path_structures/<name>.json`, git 동기화)하고,
다른 PC 에서 **원하는 목적지 폴더**(`Recreate To`) 안에 **폴더만 재생성**한다(파일 미생성). 베이스는
project_root 상대경로(`base_rel`)로도 저장되어 목적지를 자동 제안하는 데 쓰인다.
...
(**Recreate To** 확인/변경 · **Depth** 조절 · 필요 없는 폴더 체크 해제) → **Recreate**.
