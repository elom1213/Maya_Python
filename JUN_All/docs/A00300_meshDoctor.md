...
> [!info] 2026-09-22 — 이 툴은 **`A00380_MeshTool` 의 MeshDoctor 탭**으로 옮겨졌다
> 진단 로직(`mesh_scan` · `mesh_fix` · `report`)은 그대로이고 결과도 동일하다.
> **새 작업은 [`A00380_MeshTool`](A00380_MeshTool.md) §3-4 에서** 한다.
> 이 툴은 관례대로 지우지 않고 남겨 둔다 — 아래 설명은 여전히 유효하다(진단 항목·판정 기준 포함).
...
- **Target Meshes**(창 상단 리스트): 진단할 메시를 담는다.
  ...
  - **리스트에서 항목을 고르면 씬의 해당 메시도 선택된다**(v01.03~). 공용 `MOD_tsl_qt_v01` 의
    선택 연동과 같은 동작이며, 이름이 아니라 UUID 로 노드를 되찾는다. 선택이 비면 씬 선택은 건드리지 않는다.
