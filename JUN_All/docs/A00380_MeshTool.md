...
2. 바꿀 메시들을 선택 → 우측 **Targets** 의 `List Selected`. 짝 순서는 `Sort`(v01.13~) 나
   `Up` / `Down` 으로 맞춘다 (메시 오브젝트는 고른 순서대로 담긴다).

   > **`Sort` 는 양쪽을 이름순으로 정렬한다**(v01.13~). 짝은 **리스트 순서**로 맺어지므로,
   > `head_01 / head_02 …` 처럼 좌우 이름이 같은 순서로 흐르면 **양쪽 Sort 한 번으로 짝이 맞는다** —
   > `Up` / `Down` 을 수십 번 누를 일이 없어진다. 정렬해도 항목의 **UUID 는 그대로**라
   > 정렬 뒤에 메시 이름이 바뀌어도 같은 노드를 잡고, **정렬은 미리보기 세션을 버린다**
   > (옛 짝으로 확정되지 않도록 — 리스트를 바꿀 때와 같은 규칙).
...
`resize(360, 860)`. **가로는 `A00400_CurveTool` 과 같은 값(360)을 요청**한다(사용자 요청).
실제 폭은 두 툴 모두 자기 레이아웃의 최소 폭까지만 벌어진다 — 같은 규칙으로 열린다.

> [!warning] 오프스크린에서 잰 최소 폭을 창 크기로 박지 말 것
> v01.15 에서 한 번 틀렸다. 오프스크린(mayapy)으로 재면 A00400 의 최소 폭이 **1176** 으로
> 나와 그 값을 그대로 박았는데, **마야에서는 훨씬 좁게 열린다** — 오프스크린 플랫폼의 기본
> 폰트가 마야 UI 폰트보다 넓어 글자 기반 최소 폭이 전부 부풀려지기 때문이다.
> 오프스크린 수치는 **절대 px 이 아니라 비교용**으로만 쓴다(같은 조건에서 A < B 인지 정도).
> 정확한 px 이 필요하면 마야에서 `window_instance.width()` 를 찍어 확인해야 한다.
