...
- **버전**: `app/config/version.py` (v01.15 — 창 가로를 `A00400_CurveTool` 과 같은 1176 로
...
`1176 x 860` 으로 연다. **가로는 `A00400_CurveTool` 과 같은 값**이다(사용자 요청).

> [!note] `resize()` 에 적힌 숫자가 곧 창 폭은 아니다
> A00400 의 코드에는 `resize(360, 620)` 이라고 적혀 있지만, 그 툴의 **최소 폭이 1176** 이라
> 실제로는 늘 1176 으로 열린다. 그래서 "A00400 과 같은 가로" 는 코드의 360 이 아니라
> **테마를 입히고 잰 1176** 이다. 이 툴 자신의 최소는 748 x 805 이므로 1176 은 깎이지 않는다.
