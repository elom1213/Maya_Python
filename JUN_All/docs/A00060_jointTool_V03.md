...
- 버전: `v03.08` (`app/config/version.py`) — `Chain > Pole Target` 에 **`Slide`**:
  ...
  · v03.04 는 `Chain > Pole Target` 에 **`Create Selected`**:
...
A' = A + n*v           <- 여기에 오브젝트를 둔다 (n = Distance)
       + (s*0.1/2)*(p1-p3) <- 양 끝 사이를 오가는 슬라이드 (s = Slide, v03.08)
```

- `Objects (end - middle - end)` 리스트에 **정확히 3개**를 체인 순서로 담는다.
- `Distance` : 얼마나 바깥으로 밀어낼지. **뜻은 `Fixed distance` 가 정한다**(아래).
  만든 오브젝트에 `poleDistance` 어트리뷰트로 남아 **살아 있다** — 나중에 값만 바꿔도
  즉시 움직인다. 음수는 어느 모드에서나 반대쪽이다.
- **`Fixed distance` (v03.05, 기본 ON)** : 켜면 `Distance` 가 **가운데 오브젝트에서의 씬 거리**고,
  **체인을 어떻게 굽혀도 그 거리가 유지된다.** 끄면 v03.04 까지의 동작 — 굽은 정도의
  **배수**(`0` 은 중점, `1` 은 가운데 오브젝트 자신, `2` 는 두 배 바깥)라 팔을 펴면 타깃이
  팔꿈치로 빨려 들어온다.
- **`Slide` (v03.07, 배율은 v03.08, 기본 0)** : 타깃을 **양 끝 사이로** 옮긴다.
  **양수 → 리스트의 첫 오브젝트(A)와 가까워지고, 음수 → 마지막 오브젝트(C)와
  가까워진다.** `0` 은 한가운데, `+10` 은 첫 오브젝트 자리까지, `-10` 은 마지막
  오브젝트 자리까지다(한 칸 `0.1` 은 양 끝 거리의 **1/200**). 단위가 씬 거리가 아니라
  **양 끝 거리에 비례**하므로 리그 크기가 달라져도 값의 뜻이 같다.
  `poleSlide` 어트리뷰트로 남아 **살아 있다** — 씬에서 값만 바꿔도 바로 움직인다.
  `Fixed distance` 를 켜든 꺼든 둘 다 돌아간다.
  - 단, `Fixed distance` 와 같이 쓰면 가운데 오브젝트까지의 거리가 더 이상 정확히
    `Distance` 가 아니다 — 슬라이드는 마지막에 더해지는 평행이동이라 직각삼각형으로
    조금 늘어난다.
...
- `Update Selected` : 고른 폴 타깃의 `Distance` 와 `Slide` 를 바꾼다(다시 만들지 않는다).
  예전에 만든 타깃이라 `poleSlide` 가 없으면 거리만 바꾸고 **다시 지으라고 알려 준다**
  — `Update` 는 노드를 안 건드리는 것이 약속이라 여기서 몰래 배선을 고치지 않는다.
- `Bake Selected` : 컨스트레인트와 보조 노드를 지우고 **그 자리에 굳힌다**. undo 로 되돌아온다.

> **일직선이면 경고한다.** 세 점이 한 줄에 있으면 `v = 0` 이라 밀어낼 방향이 없다
> (T 포즈에서 흔하다). 막지는 않고 알려 준다 — 관절을 먼저 굽혀 두면 된다.
> 거리 고정 모드에서는 타깃이 **가운데 오브젝트 자리**에 머물고, 굽히는 순간 제자리를 찾는다.

##### 거리가 고정되는 원리 (v03.05)

예전 배선에서 타깃과 가운데 오브젝트의 거리는 `|A' - p2| = |n-1| * |v|` 라 **굽은 정도에
딸려 변했다.** 거리를 상수 `d` 로 두려면 방향만 남기고 길이를 정규화해야 한다:

```
A' = p2 + d * v / |v|        <-->        A' = A + n*v   에서   n = 1 + d / |v|
```

**두 식이 같다.** 그래서 `pointConstraint` 구성은 그대로 두고 거기에 먹이는 `n` 만 상수에서
**계산된 값**으로 바꾸면 끝난다 — 월드/부모 공간 변환은 여전히 컨스트레인트가 공짜로 해 준다.
`|v|` 는 `decomposeMatrix` 둘 + `plusMinusAverage`(average) + `distanceBetween` 으로 잰다.

- **`multiplyDivide` 의 0 나누기는 `100000` 을 돌려준다**(실측, 경고만 낸다). 더 위험한 것은
  `|v|` 가 0 이 아니라 **아주 작을 때** — `n` 이 1e9 쯤으로 뛰면 가중 평균이 큰 수끼리의
  뺄셈이 되어 자릿수가 날아간다. 그래서 나누기 앞에 `clamp` 로 하한을 잡는다.
- **모드를 바꾸고 `Create Selected` 를 다시 누르면 다시 짓는다.** 체인이 같아도 배선 모드가
  다르면 `kept` 가 아니다 — 안 그러면 체크박스를 켜도 아무 일이 없는 것처럼 보인다.
  이때 컨스트레인트만 지우면 계산 노드가 떠돌이로 남으므로, 가중치에서 거슬러 올라가
  **우리가 만든 유틸리티 노드만** 골라 함께 지운다(`Bake Selected` 도 같은 경로).
- **`A00130_ControlRig_V02` 의 템플릿 폴 타깃은 영향을 받지 않는다** — 그쪽이 쓰는
  `ensure()` 의 기본값은 **배수**다.

##### 양 끝 사이로 미끄러뜨리는 원리 — `Slide` (v03.07)

폴 타깃은 방향만 맞으면 된 것이 아니라 **체인을 따라 앞뒤로도** 놓고 싶다
(무릎 타깃은 골반 쪽으로, 팔꿈치 타깃은 어깨 쪽으로). 그래서 현(chord) 방향으로
평행이동하는 두 번째 어트리뷰트를 둔다:

```
A' = A + n*v + (s/2)*(p1 - p3)
   = ((1-n)/2 + s/2)*p1  +  n*p2  +  ((1-n)/2 - s/2)*p3
     \________________/            \________________/
        w0                             w2          (합 = (1-n) + n = 1)
```

**가중치 합이 여전히 1 이라 컨스트레인트 구성이 그대로다.** `s/2` 는 새 노드 없이
이미 있던 `_side`(multiplyDivide) 의 **Y 채널**로 계산한다 — 한 노드가 세 채널을 갖고
`operation`(divide) 은 세 채널에 같이 걸리므로, 늘어난 노드는 가중치를 더하고 빼는
`plusMinusAverage` 둘뿐이다(`_weightA` · `_weightC`).

- ★ **슬라이드를 `n` 에 섞지 않는 것이 핵심이다.** 기준점 `A` 를 먼저 움직인 뒤
  거기서 `n*v` 를 가면(즉 `A_s + n(p2 - A_s)`) 가중치가 `(1-n)*s/2` 가 되어
  **`n > 1` 에서 부호가 뒤집힌다.** 폴 타깃은 보통 `n > 1` 이므로
  "양수를 넣었는데 C 쪽으로 간다" 가 된다. 슬라이드는 **마지막에 더하는 평행이동**이어야 한다.
- 단위를 **반현에 비례**하게 잡은 이유: 씬 거리로 두면 리그 크기마다 값을 다시 잡아야 한다.
- **거기에 배율 `0.1` 을 곱한다 (v03.08).** 반현을 그대로 단위로 쓰면 스핀박스 한 칸(0.1)이
  반현의 10% 라 **살짝 돌려도 크게 움직인다.** 곱하고 나면 한 칸은 반현의 1%,
  `±10` 이 "끝까지" 다. 노드는 늘지 않는다 — `_side` 의 Y 채널 **나눗수를 `2` 에서 `20`
  으로** 바꾸면 반으로 나누기와 배율이 한 번에 처리된다.
  - **예전 배율로 배선된 타깃은 `Create Selected` 가 한 번 다시 짓는다.** 그냥 두면
    **같은 씬 안에서 타깃마다 감도가 달라져** 찾기 어려운 어깋남이 된다. 버전이나 이름이
    아니라 **살아 있는 나눗수 값**을 읽어 판정한다(`slide_scale_of()`).
- **예전에 만들어 둔 타깃은 `Create Selected` 를 한 번 누를 때 다시 짓는다.**
  `poleSlide` 가 없으면 구성이 달라서 `kept` 로 둘 수 없기 때문이다 — 그대로 두면
  버튼을 눌러도 슬라이드가 생기지 않는다. 재배선은 한 번이고 그 뒤로는 `kept` 다.
