...
- 버전: `v03.07` (`app/config/version.py`) — `Chain > Pole Target` 에 **`Slide`**:
  타깃을 **양 끝 사이로** 옮긴다(양수 = 첫 오브젝트 쪽, 음수 = 마지막 쪽) (§Pole Target)
...
A' = A + n*v           <- 여기에 오브젝트를 둔다 (n = Distance)
       + (s/2)*(p1-p3) <- 양 끝 사이를 오가는 슬라이드 (s = Slide, v03.07)
```
...
- **`Fixed distance` (v03.05, 기본 ON)** : 켜면 `Distance` 가 **가운데 오브젝트에서의 씬 거리**고,
  ...
  **배수**(`0` 은 중점, `1` 은 가운데 오브젝트 자신, `2` 는 두 배 바깥)라 팔을 펴면 타깃이
  팔꿈치로 빨려 들어온다.
- **`Slide` (v03.07, 기본 0)** : 타깃을 **양 끝 사이로** 옮긴다.
  **양수 → 리스트의 첫 오브젝트(A)와 가까워지고, 음수 → 마지막 오브젝트(C)와
  가까워진다.** `0` 은 한가운데, `+1` 은 첫 오브젝트 자리까지, `-1` 은 마지막
  오브젝트 자리까지다. 단위가 **반현(양 끝 거리의 절반)** 이라 리그 크기가 달라져도
  값의 뜻이 같다. `poleSlide` 어트리뷰트로 남아 **살아 있다** — 씬에서 값만 바꿔도 바로 움직인다.
...
- 단위를 **반현**으로 잡은 이유: 씬 거리로 두면 리그 크기마다 값을 다시 잡아야 하고,
  `±1` 이 "끝까지" 라는 뜻이라 감이 바로 온다.
