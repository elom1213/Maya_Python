...
## 5-1. Auto Backup — 저장 후 지연 백업 (v01.05, 지연 v01.08)

`Control` 의 **Auto Backup** 체크박스(기본 켜짐)는 백업을 *주기*가 아니라 *저장 시점*에 맞춘다.

| Auto Backup | 동작 |
|-------------|------|
| **켜짐**(기본) | 대상 파일을 실시간 감시(`QFileSystemWatcher`)하다가 **저장이 감지되면 `Save Delay` 초 뒤에** 그 파일만 백업한다. 바뀌지 않은 파일은 건드리지 않는다. 카운트다운 대신 `Auto save` 표시. |
