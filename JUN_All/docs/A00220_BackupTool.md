...
```
                             [ Shrink ] [ Pin ]  │  ← 우상단 토글 : Shrink (v01.15) / Always on Top (v01.13)
┌ Target Files ─────────────────────────────────┐
│ C:/work/sceneA.mb                              │
│ C:/work/charB/rig.ma                           │
...
- **Pin (Always on Top, v01.13~)**: 우상단의 체크형 `Pin` 버튼을 켜면 이 창이 다른 창들보다 **항상
...
- **Shrink (v01.15~)**: `Pin` 왼쪽의 체크형 `Shrink` 버튼. 켜면 **공룡만 남기고** Target Files ·
  ...
  - **백업은 그대로 돈다** — 감추는 것은 화면뿐이라, 줄여 둔 채로 저장 톡 점프(§5)와 백업 스핀을 본다.
  - 다시 누르면 **줄이기 전 높이**로 돌아오고, 접어 둔 `Settings` 는 접힌 채로 남는다.
  - 구현 메모: 공룡은 `Control` 그룹 안에 있어 그룹을 숨기면 함께 사라진다. 그래서 줄이는 동안
    공룡을 **창 레이아웃으로 옮겼다가**(복제가 아니라 이동 — 애니메이션·타이머가 끊기지 않는다)
    원래 자리로 돌려놓는다. 숨긴 뒤에는 레이아웃 최소 높이를 다시 계산해야 창이 줄어든다
    (`Settings` 접기와 같은 이유).
