...
6. **`Length` 탭 > `Measure`** — 템플릿 조인트 사이 거리를 잰다. **씬은 안 바뀐다.**
7. **`Length` 탭 > `Write Values`** — 그 거리를 옵션 컨트롤러에 쓴다. **undo 한 스텝.**

> **`Orient` 는 `Match` 보다 먼저** 돌린다 — Match 는 케이지를 조인트 자리로 옮기므로
> 방향이 정해진 뒤여야 한다. 3번(손으로 배치) 다음에 `Orient` 탭 > `Orient` 를 누른다.
