...
- **`Interpolation` 을 `Bezier` 로 두면 포인트마다 탄젠트 핸들**이 붙어 포인트 사이가
  곡선이 된다(v01.07~). 핸들은 드래그로도 `Tangent` 줄의 **각도 / 길이** 숫자로도 조절하고,
  **`Break`** 로 좌우를 독립시킨다. "루트 근처는 완만하게, 팁에서 급히" 같은 감쇠를 만들 때 쓴다.
