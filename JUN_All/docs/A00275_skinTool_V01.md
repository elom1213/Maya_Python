...
| **Select** (v01.27~) | 스킨 웨이트로 버텍스를 고른다 (**선택 말고는 씬이 바뀌지 않는다**) | **By Weight** | 메시(전체) 또는 그 버텍스 일부를 불러오면 바인드된 조인트가 체크박스와 함께 리스트업된다. 체크한 조인트의 웨이트가 **Value(0~1) 이상 / 이하**인 버텍스를 그 범위 안에서 선택 |
...
## Weights > Copy Weights — 한 메시 안에서 버텍스 → 버텍스 (v01.17~)

**같은 메시 M** 의 버텍스 집합을 복사해 두고, 나중에 M 의 **다른 버텍스들을 골라** 그 웨이트를
그대로 붙여넣는다. 대칭이 아닌 자리, 루프 한 줄만 다시 칠하고 싶은 자리, 형상을 고친 뒤 그
주변만 되살리고 싶은 자리에 쓴다.

### 사용법

1. **복사할 버텍스를 선택** → `Copy (store vertices from selection)`.
   라벨이 `Copied: 1234 vertices (head_geo)` 로 바뀐다. **이 단계에서는 아무것도 쓰지 않는다.**
2. `Search mode` 를 고른다 (아래).
3. `Blend` 로 **얼마나 실을지** 정한다 (아래, 기본 `1.0` = 전부).
4. **붙여넣을 버텍스를 선택** → `PASTE weights onto selected vertices`.

> 복사한 버텍스도 **리스트 위젯으로 펼치지 않는다** — 개수 라벨 + `Select` / `Clear` 뿐이다.
> 한 덩이가 수천 개인 것이 예사라 그만한 리스트를 만들면 창이 바로 느려진다
> (Expand Bind 와 같은 이유).

### 무엇이 "그대로 복사" 인가

목표 버텍스마다 **가장 가까운 소스 버텍스 하나**를 골라, 그 버텍스의 **웨이트 행 전체**를
통째로 베낀다. **값을 섞거나 보간하지 않는다.**

- 인플루언스가 몇 개든, 행의 합이 1 이 아니든 소스와 **완전히 같은 값**이 나온다
  (`normalize=False` — 여기서 다시 정규화하면 그것은 이미 "그대로" 가 아니다).
- 인플루언스를 새로 넣지 않는다. 소스와 목표가 **같은 skinCluster** 라 열 구성이 이미 같다.
  그래서 이 기능은 **한 메시 안에서만** 동작한다 — 다른 메시를 고르면 어느 메시에서 복사했는지
  적어서 거절한다. 메시 사이 전이는 `Transfer` / `Migrate A -> B` 쪽이다.
- 소스 자신에게 붙여넣으면 자기 자신이 최근접이라 **값이 변하지 않는다**(안전한 no-op).

이것은 `Blend = 1.0`(기본) 일 때의 이야기다. 아래 `Blend` 를 내리면 그만큼만 실린다.

### Blend — 얼마나 실을지 (0~1, v01.18~)
...
| Blend | 결과 |
|-------|------|
| **1.0** (기본) | 소스 행이 목표 행을 **덮어쓴다** — v01.17 과 완전히 같다 |
| **0.5** | **절반만** 실린다. 목표가 원래 갖고 있던 웨이트를 절반 유지 |
| **0.0** | 아무것도 바뀌지 않는다(로그로 알린다) |

예: 목표가 `(0.5, 0.5)`, 소스가 `(0.9999, 0.0001)` 일 때
`Blend 0.5` → `(0.74995, 0.25005)`, `Blend 0.25` → `(0.62498, 0.37503)` (실측).

> [!tip] Blend 는 정규화를 깨지 않는다
> 합이 1 인 두 행의 **볼록결합**은 다시 합이 1 이다. 그래서 소스와 목표가 둘 다 정규화돼
> 있었다면 결과도 정규화돼 있고, `normalize=False` 를 그대로 둘 수 있다
> (여기서 마야에게 다시 정규화를 맡기면 그것은 이미 "그대로" 가 아니다).

- 범위 밖 값은 잘라 낸다(`clamp01`) — `-1` 은 `0`, `2` 는 `1` 로 보고 로그에 실제 값을 적는다.
- **Blend 가 1 이면 목표의 원래 웨이트를 읽지 않는다.** 어차피 전부 덮어쓰므로 읽는 비용
  (버텍스 × 인플루언스)이 그대로 낭비다 — 기본값 경로는 v01.17 과 **같은 값·같은 비용**이다.
- 닿지 못한 버텍스를 손대지 않는 규칙은 Blend 와 무관하게 그대로다.

### Search mode — Expand Bind 의 Falloff mode 와 같은 세 가지

| 모드 | 무엇을 재나 | 쓰는 때 |
|------|-------------|---------|
| **Surface (edge length)** | 메시의 엣지를 따라가는 **측지 거리** | 기본. 표면을 따라 이어진 자리 |
| **Topology (edge count)** | 엣지 **개수**(홉 수) — 길이 무시 | 엣지 간격이 들쭉날쭉해도 "몇 칸 떨어졌나" 로만 세고 싶을 때 |
| **Volume (straight line)** | **직선 거리** — 토폴로지 무시 | 틈 건너편/다른 조각에서 가져와야 할 때 |

같은 자리에서도 답이 갈린다. 열 간격이 넓고 세로 간격이 좁은 격자에서, 목표 하나를 두고:

| 모드 | 고른 소스 | 이유 |
|---|---|---|
| Surface | 가까운 쪽 | 엣지 길이의 합이 짧다 |
| Topology | **먼 쪽** | 길이는 안 보고 홉 수만 센다 — 먼 쪽이 2홉, 가까운 쪽이 3홉 |
| Volume | 가까운 쪽 | 직선 거리 |

- **Surface / Topology 는 메시를 걸어간다.** 그래서 선택한 버텍스가 **떨어진 조각**에 있으면
  소스에 닿지 못한다. 그런 버텍스는 **손대지 않고** 몇 개였는지 로그에 남긴다. 하나도 못 닿으면
  아무것도 쓰지 않고 `Volume` 을 써 보라고 알린다 — 조용히 다른 모드로 넘어가지 않는다.
- **Volume 은 언제나 소스를 찾는다.** 대신 틈 건너편에서 가져올 수 있다.
- 탐색 범위는 **메시 전체**다. Expand Bind 는 저장한 집합 **안으로** 인접을 가두지만
  (falloff 가 영역 밖으로 새면 안 되니까), 여기서는 소스와 목표가 **떨어져 있는 것이 정상**이라
  가둬 두면 아무 데도 못 간다.

### 구현 메모

- surface / topology 는 `expand_bind_manager.dijkstra_multi` 를 그대로 쓴다. 소스 집합 전체를
  시작점으로 **한 번** 퍼뜨리면 버텍스마다 "가장 가까운 소스가 누구인지"(anchor)가 한 번에
  나오므로 소스마다 따로 돌릴 필요가 없다.
- **volume 은 전수 비교를 하면 안 된다.** 목표마다 소스를 전부 재면 `목표 × 소스` 다 —
  22,500 정점 메시에서 소스 5,000 / 목표 15,000 이면 순수 파이썬으로 **약 20초**다. 그래서
  소스를 **균일 격자**에 담고 목표 주변 셀만 겹겹이 넓혀 가며 본다. "다음 겹의 가장 가까운
  지점보다 이미 찾은 것이 더 가깝다" 가 되면 멈춘다 — **근사가 아니라 전수 비교와 같은 답**이고
  (300 샘플에서 거리 차 `0.0e+00` 실측) **1.00초**로 20배 빠르다.
  소스가 64개 미만이면 격자를 만드는 비용이 더 크므로 그냥 전수 비교한다.
- 소스 웨이트는 **실제로 쓰인 것만** 읽는다(소스 1000개를 골라도 목표가 10개면 10행).
- `Blend` 가 1 미만일 때만 목표의 현재 웨이트를 한 번 더 읽어 섞는다. 읽는 컴포넌트는
  쓸 때와 **같은 것**이라 행 순서가 어긋날 여지가 없다.
- 실행 전체가 **undo 한 스텝**으로 묶인다.

---

## Weights > Layer — 여러 메시의 웨이트를 레이어처럼 합성 (v01.22~)

**버텍스 순서가 같은 메시 N 개**(M_01, M_02, M_03 …)가 각자 다른 조인트에 바인드돼 있을 때,
메시마다 **가져올 조인트(lock)** 와 **가져올 양(Blend)** 을 정해 한 메시의 웨이트로 합친다.
결과는 **새 메시**로 만들거나(`Create new mesh`) **이미 있는 메시의 웨이트만 갱신**한다
(`Update existing mesh`). 계획서: [plans/A00275_skinTool_V01_layer_tab_plan.md](plans/A00275_skinTool_V01_layer_tab_plan.md).

### 사용법

1. 스킨된 메시들을 **위에 올 순서대로** 선택 → `Add Selected`. 리스트 **맨 아래가 베이스**다
   (`#` 칸에 `base`). 순서는 `Up` / `Down` 으로 바꾼다.
2. 레이어 줄을 고르면 아래 `Joints` 에 그 메시에 바인드된 조인트가 나온다. **lock 할 조인트를 체크**한다.
   - 행을 클릭 / **Shift+클릭**(범위) / **Ctrl+클릭**(토글)으로 여러 개 고른 뒤, 그중 **하나의 Lock 칸을
     누르면 선택된 행 전부**가 같이 바뀐다(선택은 그대로 남는다). `Space` 도 같다.
     A00290 Shape Editor 의 다중 편집과 같은 조작이다.
   - `Lock Selected` / `Unlock Selected`, 필터에 보이는 행만 `Lock All` / `Unlock All`.
3. 레이어마다 `Blend`(0~1)를 정한다.
4. `Output` 에서 `Create new mesh`(이름) 또는 `Update existing mesh`(대상 — `<- Set`) → **MERGE**.

### 합성 규칙

버텍스마다 lock 값을 **아래 레이어부터** 붓는다 — 합이 1 을 넘으면 **위 레이어부터 잘린다**.

```
cap = 1                                  # 아직 남은 몫
lock 이 있는 레이어마다 (아래 -> 위):
    c = Blend x (lock 한 조인트의 웨이트)   # 절대값 그대로
    합(c) <= cap : 그대로 넣고 cap -= 합(c)
    합(c) >  cap : c 를 cap/합(c) 배로 줄여 넣는다 (넘친 레이어만 잘린다)
베이스에 lock 이 없으면 → 베이스의 전체 웨이트로 남은 cap 을 채운다
그래도 cap 이 남으면 → 재정규화해서 합을 1 로
```

> [!note] v01.23 에서 방향을 바로잡았다
> v01.22 는 lock 을 **위 레이어부터** 부어서 넘치면 **아래 레이어가** 잘렸다. 그래서 위 레이어가 조인트를
> 전부 lock 하면(합 1.0) 아래 레이어의 lock 이 **통째로 사라지고 경고도 없었다**(베이스의 잘림은 세지
> 않았다). 계획서 Q3 답("합이 1 넘긴 부위만 위 레이어에서부터 자르도록")을 반대로 읽은 것이다.
> 예: 위 `M_02`(jnt_02_a, jnt_02_b 전부 lock) / 베이스 `M_01`(jnt_01_a lock) → v01.22 는 jnt_01_a 가
> 한 버텍스에도 안 남았고, v01.23 은 **jnt_01_a 가 M_01 값 그대로**, jnt_02_a/b 가 나머지를 M_02 비율로 채운다.
> lock 합이 1 을 넘지 않는 경우(처음 예시, Q1, Q2)는 두 버전 결과가 같다.

| 경우 | 결과 |
|---|---|
| **lock 값은 절대값으로 보존** | 합이 1 을 넘는 버텍스에서만 **아래 레이어가 먼저 보존되고, 위 레이어부터** 비율대로 줄어든다 |
| lock 한 조인트가 전부 0 인 버텍스 | 그 레이어가 아무것도 안 가져가므로 **아래 레이어의 웨이트가 그대로** 옮겨진다 |
| **베이스에 lock 이 없다** | 베이스의 조인트 **전부**로 나머지를 채운다 |
| 베이스에도 lock 을 걸었다 | 그 조인트만 쓰고, 1.0 에 못 미친 버텍스는 **재정규화** |
| 아무 레이어도 기여하지 못한 버텍스 | 베이스의 **전체 웨이트**를 쓴다(로그로 개수) |
| 베이스가 아닌 레이어에 lock 이 없다 | 그 레이어는 아무것도 더하지 않는다(경고) |

숫자 예 (Blend 전부 1):

| | M_01 (lock `01_a`,`01_b`) | M_02 (베이스) | 결과 |
|---|---|---|---|
| 버텍스 A | a .5 / b .2 / c .3 | a .1 / b .6 / c .3 | `01_a .5 · 01_b .2 · 02_a .03 · 02_b .18 · 02_c .09` |
| 버텍스 B | a 0 / b 0 / c 1 | M_02 행 | **M_02 행 그대로**, `01_c` 는 없다 |

메시 3개 — M_01 `01_a`(.4) lock, M_02 `02_b`(.5) lock, M_03 베이스 `03_a` 1.0 → `01_a .4 · 02_b .5 · 03_a .1`.

> [!warning] lock 합이 1 을 넘으면 **순서가 결과를 바꾼다**
> 그런 버텍스에서는 **아래에 있는 레이어가 자기 lock 값을 다 지키고**, 위 레이어가 남은 만큼만 받는다.
> 예를 들어 `01_a .8` 과 `02_b .7` 을 lock 하면 M_01 이 위일 때 `02_b .7 / 01_a .3`, M_02 가 위일 때
> `01_a .8 / 02_b .2` 다. 메시 2개여도 같다 — 위 레이어가 조인트를 전부 lock 해도 **아래(베이스)의 lock 이
> 먼저** 들어간다. 그런 버텍스 수는 로그에 `the upper layers were cut` 로 나온다.

### 결과 메시

- **형상은 베이스 메시의 rest(skinCluster 입력) 형상**이다. 화면의 변형된 형상을 복제하면 지금 포즈가
  구워지므로 쓰지 않는다. 위치(월드 행렬)도 베이스를 따른다.
- **머티리얼은 면 단위 배정까지 베이스와 똑같이 붙는다 (v01.25~).** 그전에는 베이스에 붙은 셰이딩
  그룹 중 **첫 번째 하나**를 메시 전체에 걸어서, 머티리얼이 5개 붙은 메시로 만들어도 M_new 는
  **한 벌만 입은 채** 나왔다. `listConnections` 로 얻는 그 목록에는 *어떤 면에 무엇이 붙었는지*가
  없기 때문이다. 지금은 `MFnMesh.getConnectedShaders` 로 **면별 배정**을 읽어 그대로 옮긴다 —
  머티리얼 5 / 3 / 1 개 메시에서 **면별 배정이 원본과 완전히 일치**하는 것을 실측했다.
  (머티리얼이 하나면 면 단위가 아니라 셰이프째 건다. 면 개수가 다르면 손대지 않고 경고를 남긴다.)
- 인플루언스는 결과 웨이트가 0 이 아닌 조인트뿐이다(예: M_01 의 `01_c` 는 들어가지 않는다).
- **`bindPreMatrix` 는 그 조인트를 가진 가장 위 레이어의 skinCluster 에서 복사한다.** 새로 바인드하면
  마야는 *지금* 조인트 위치를 바인드 포즈로 삼으므로, 리그가 포즈된 상태에서 Merge 하면 메시가 튄다.
  복사하면 포즈 중에 Merge 해도 그대로다(실측). 같은 조인트가 두 레이어에서 다른 바인드 행렬을 갖고
  있으면 위 레이어 값을 쓰고 경고한다.
- 조인트가 바인드 포즈가 아닐 때 만든 결과에는 `bindPose` 노드를 남기지 않는다(생성 시점 포즈로 잘못
  저장되므로). 로그로 알린다.
- **메시들은 공간상 겹쳐 있어야 한다.** 조인트 개수·위치는 달라도 된다. 결과 메시는 모든 조인트를
  동시에 따르므로, 소스 메시가 옆으로 떨어져 있으면 그 스켈레톤의 피벗을 중심으로 돌아 틀어진다.

### Update existing mesh

- 대상은 **레이어가 아닌** 메시여야 하고 버텍스 수가 같아야 한다.
- 대상에 skinCluster 가 없으면 새로 만든다. 있으면 **없는 조인트만 추가**(바인드 행렬은 레이어에서
  복사)하고, 원래 있던 조인트 중 결과가 쓰지 않는 것은 **웨이트 0 으로 남긴다**(로그에 이름).
  이미 있는 조인트의 바인드 행렬이 레이어와 다르면 대상 것을 그대로 두고 경고한다.
- **Ctrl+Z 한 번**이면 이전 웨이트와 추가한 조인트가 그대로 돌아온다.

### 구현 메모

- 코어: `app/core/weight_layer_manager.py` (`compose_rows` 는 마야 없는 순수 계산), UI: `app/ui/layer_tab.py`.
- **웨이트는 API `setWeights` 가 아니라 버텍스마다 구간 `setAttr` 로 쓴다.** `MFnSkinCluster.setWeights` 는
  undo 기록에 남지 않는다(청크 안에서 써도 `There are no more commands to undo` — mayapy 실측). 갱신
  모드는 Ctrl+Z 로 되돌아가야 해서 두 모드 모두 `weightList[v].weights[lo:hi]` 로 쓴다. 구간은 **새 값과
  지금 들어 있는 값의 인덱스를 모두** 덮는다 — 안 그러면 예전 인플루언스 웨이트가 남아 합이 1 을
  넘는다. 19,881 버텍스 x 8 인플루언스에 0.57초, undo 0.27초. `normalizeWeights` 가 켜져 있어도 직접 쓴
  값은 건드리지 않는다.
- 레이어 웨이트는 API `getWeights` 로 한 번에 읽는다. 합이 1 이 아닌 행은 읽을 때 정규화하고 알린다.
- Lock 리스트 항목에는 `ItemIsUserCheckable` 을 **주지 않는다** — 주면 델리게이트가 마우스를 **놓을 때**
  체크를 한 번 더 뒤집어 한 번 누르면 두 번 바뀐다. 전환은 트리가 누를 때 직접 하고, 선택은 건드리지 않는다.
- 성능: 19,881 버텍스 x 레이어 3 x 조인트 8 **1.0초**.
- 레이어 줄은 `setCurrentItem` 으로 고른다. `setSelected(True)` 는 단일 선택 목록이어도 다른 줄을 해제하지
  않아, 두 줄이 선택된 채 엉뚱한 레이어가 현재 레이어로 읽힌다.
- 검증(mayapy): 코어 47항목 — 위 표의 숫자들, 2레이어 전 버텍스 식, 순서에 따른 결과 차이, **포즈 중 Merge 후
  디포메이션 일치**(lock=1 영역은 M_01, 0 영역은 M_02 와 1e-4 이내, 포즈를 더 바꿔도 유지), Update 모드
...
## Select > By Weight — 웨이트로 버텍스 고르기 (v01.27~)

스킨된 메시 M 에서 **체크한 조인트의 웨이트가 기준값 이상 / 이하인 버텍스**를 선택한다.
메시 전체에서 찾을 수도 있고, **M 의 버텍스 일부만** 불러와 그 안에서만 찾을 수도 있다. 씬은 선택 말고 아무것도 바뀌지 않는다.

### 사용법

1. **범위 불러오기** — 메시 M 을 선택하거나(전체), M 의 버텍스 일부를 선택하고(엣지·페이스도 버텍스로 바뀜)
   **Load Mesh / Vertices**. 라벨에 `M  -  61 of 220 vertices  (skinCluster1)` 처럼 범위가 뜬다.
   - 범위는 **불러올 때 저장**된다. 결과를 고르면 마야 선택이 결과로 바뀌고, 조인트 행을 눌러도 조인트가
     선택되기 때문이다 — 그래도 범위는 그대로라 **Value 만 바꿔 다시 고를 수 있다.**
...
3. **Condition**
   ...
   - `Several joints` — 여러 조인트를 체크했을 때
     - **Any checked joint**(기본) : 하나라도 조건을 만족
     - **All checked joints** : 모두 만족
     - **Sum of checked joints** : 웨이트 **합**이 조건을 만족
4. **Select Vertices** — 범위 안에서 맞는 버텍스를 선택한다(선택 교체). 맞는 버텍스가 없으면 **선택을 바꾸지 않고** 경고만 남긴다.

> `<=` 는 그 조인트에 **전혀 묶이지 않은(웨이트 0) 버텍스도** 포함한다 — `Weight <= 0.0` 이 곧 "영향 없는 버텍스" 찾기다.

### 구현 메모

- 코어 `app/core/weight_select_manager.py`, UI `app/ui/weight_select_tab.py`(main_window 가 길어서 Layer 탭처럼 위젯 하나).
- 웨이트는 `MFnSkinCluster.getWeights` **한 번**으로 읽는다(물리 인덱스 열 순서). 19,802 버텍스 전체에서 0.03초.
- 판정 `match_rows()` 는 마야 없는 순수 계산이고, 결과는 연속 구간(`M.vtx[a:b]`)으로 묶어 선택한다.
- 조인트 항목에는 UUID 를 따로 붙여 두어, 불러온 뒤 조인트 이름을 바꿔도 같은 조인트로 찾는다.
- 체크박스 클릭은 리스트 viewport 의 eventFilter 가 직접 처리한다. QListWidget 기본 처리에 맡기면 체크 전파는 되지만
  **고른 행이 누른 행 하나로 풀려**, 다음 클릭부터 한 행씩만 바뀐다(오프스크린 QTest 실측).
- 불러온 버텍스 id 가 지금 메시 버텍스 수를 넘으면(토폴로지 변경) 다시 불러오라고 거절한다.
- 검증(mayapy 2024 + 오프스크린 Qt) **43항목 통과** — 메시 전체 `>=`/`<=` x 기준값 4개가 `skinPercent` 로 센 답과 일치,
  부분 범위(버텍스+페이스 선택)에서 범위 밖이 섞이지 않음, Any/All/Sum, 에러 5종(조인트 없음 · 바인드 안 된 조인트 ·
  범위 밖 값 · 토폴로지 불일치 · 스킨 없는 메시), UI: 카테고리/하위 탭, 체크 가능 항목, **실제 마우스 클릭으로 다중 선택 체크 토글 + 선택 유지**,
  스핀·슬라이더 동기, 선택이 바뀐 뒤에도 저장한 범위로 실행, 재로드 시 체크 유지, 조인트 리네임 후 실행.
---
...
4. `Falloff mode` 와 `Soft Select` 반경을 정한다. **`Fit to Joints`** 를 누르면 각 조인트의 falloff 가
   ...
   > **조인트 간격이 비균일하거나 밴드 폭이 자리마다 다르다면 `Even distribution` 을 켠다**(v01.16~).
   > 반경 두 개를 쓰지 않고 **자리**로 분배한다 — 아래 §Even distribution.
...
### Even distribution (v01.16~) — 간격·폭이 제각각이어도 같은 분배

위 방식의 반경 두 개(`Soft Select` / `Across width`)는 **씬 단위 절대 거리**다. 그래서 자리마다
간격이나 폭이 다르면 **분배 모양도 자리마다 달라진다.** `Even distribution` 체크박스는 두 방향 모두
거리 대신 **자리(파라미터)** 로 재서 이것을 없앤다. **엣지 루프가 있어야 켤 수 있다.**

| 방향 | 무엇이 커브의 0~1 이 되나 | 결과 |
|------|---------------------------|------|
| 루프를 **따라** | 이웃한 **두 조인트 사이 구간** 전체 | 간격이 비균일해도 분배 모양이 같다. **조인트가 앉은 버텍스는 그 조인트가 웨이트 1** 이고 다른 조인트는 0 |
| 루프에서 **멀어지며** | 그 자리의 **밴드 폭** (루프 → 밴드 끝) | 폭이 좁은 줄과 넓은 줄의 분배가 **똑같아진다** |

**켜면 `Soft Select` · `Across width` · `Fit to Joints` 가 회색으로 꺼진다.** 계산에서 실제로 빠지기
때문이다("비활성인데 값은 쓰인다"를 만들지 않는다). 다섯 가지 반경 조합(0.0001 ~ 1000)으로 돌려
결과 웨이트가 **완전히 동일**한 것을 헤드리스로 실측했다.

**예 — xy 평면 격자, 조인트 3개가 x 로 비균일 배치(간격 1.0 / 3.0), 열마다 +y 폭이 다름(1.0 / 1.5 / 3.0).**
루프는 `[vtx_01_a, vtx_02_a, vtx_03_a]`, Linear 커브. 열을 따라 `a → b → c` 로 읽은 조인트 웨이트:

| | vtx_*_a | vtx_*_b | vtx_*_c |
|---|---|---|---|
| **off** — 폭 1.0 인 열 (e_01) | 0.600 | 0.500 | 0.400 |
| **off** — 폭 3.0 인 열 (e_03) | 1.000 | 0.500 | 0.000 |
| **on** — 폭 1.0 인 열 | **1.000** | **0.500** | **0.000** |
| **on** — 폭 3.0 인 열 | **1.000** | **0.500** | **0.000** |

끈 상태에서는 `jnt_01` 이 `vtx_02_a` 까지 **0.400** 을 가져가고 자기 버텍스는 0.600 밖에 못 받는다.
켜면 커브 값 그대로 `1.000 / 0.500 / 0.000` 이 되어 **커브 가로축 왼쪽 = `vtx_*_a`, 오른쪽 = `vtx_*_c`**
로 읽힌다. **열린 루프와 닫힌 루프 모두** 같은 결과다(닫힌 루프는 마지막 조인트 → 첫 조인트를
감아 도는 구간까지 이어진다).

- **조인트가 앉은 버텍스의 웨이트 1 은 커브 모양과 무관하게 못박는다.** 끝값이 1 인 커브
  (`Solid` 프리셋)에서는 정규화만으로는 이웃 조인트가 같이 딸려 오기 때문이다. 두 조인트가
  **같은 버텍스로 스냅**했다면 누구 것인지 정할 수 없으므로 손대지 않고 반반으로 둔다.
- **밴드 폭은 anchor 별로** 잰다 — 그 루프 버텍스에서 뻗어 나간 **가장 먼 거리**다. 폭을 알아야
  하므로 이 모드의 바깥 방향 탐색은 **반경으로 자르지 않고** 밴드 끝까지 간다.
- 열린 루프에서 **조인트가 덮지 않는 양 끝** 구간은 끝 조인트가 그대로 잡는다. 여기서 감쇠시키면
  그 구간이 어느 조인트에도 안 잡혀 바인드가 비기 때문이다.
- 조인트 사이 분배는 **합이 1 이 되도록 미리 정규화**한다. 그래서 좌우 비대칭 커브
  (`Ease In` / `Ease Out` / `Spike`)에서도 조인트 사이가 덜 덮이지 않고, `coverage` 는 바깥 방향
  커브만 따른다.
- 루프 없이 켜면 **조용히 무시하지 않고 거절한다** — '따라/바깥' 두 방향을 가를 기준이 없기 때문이다.
...
Copy Weights (v01.17) — 30항목:

- **그대로 복사** — 목표 7개가 각자 자기 열의 소스 행과 **값까지 동일**(정규화되지 않은 행 포함),
  6 인플루언스 행도 열 순서 그대로. 소스 자신에게 붙여넣으면 값이 변하지 않는다(no-op)
- **세 모드가 실제로 다른 소스를 고른다** — 열 간격이 넓은 격자에서 같은 목표를 두고
  Surface/Volume 은 가까운 쪽, **Topology 는 홉 수가 적은 먼 쪽**을 고른다
- **떨어진 조각** — Surface 는 하나도 못 닿고 **아무것도 쓰지 않은 채** `Volume` 을 쓰라고
  알리며 거절, Volume 은 전부 붙여넣고 값이 소스와 동일
- **격자 최근접이 전수 비교와 같은 답** — 소스 3 / 70 / 400 / 1599 네 경우, 목표 200개에서
  거리 차 `0.0e+00`. 목표가 소스 bbox **바깥**에 있는 경우도 동일
- **성능** — 22,500 정점에서 소스 5,000 / 목표 15,000 volume 이 **1.00초**(전수 비교 환산
  약 20초 → **20배**), surface 는 같은 규모 end-to-end **0.14초**
- **방어** — 소스 없음 · 목표 없음 · 토폴로지 바뀐 stale id · skinCluster 없는 메시 ·
  다른 메시에 붙여넣기(UI) 전부 명확한 에러

Even distribution (v01.16) — 43항목:

- **요청 1 (폭이 달라도 같은 분배)** — 폭 `1.0 / 1.5 / 3.0` 인 세 열이 전부
  `1.000 / 0.500 / 0.000` 으로 **완전히 동일**하고, 그 값이 Linear 커브 값 그대로다
  (= 커브 가로축 왼쪽이 `vtx_*_a`, 오른쪽이 `vtx_*_c`). 행 배치가 비례하지 않아도
  (`0 / 0.25 / 1`) 두 열이 같다
- **요청 2 (조인트 버텍스 웨이트 1)** — 조인트 간격이 비균일(`1.0` / `3.0`)해도
  `vtx_01_a → jnt_01`, `vtx_02_a → jnt_02`, `vtx_03_a → jnt_03` 이 전부 **1.000000000**,
  나머지 조인트는 **0.000000000**. 끝값이 1 인 `Solid` 커브에서도 유지
- **대조군** — 같은 씬을 `even=False` 로 돌리면 두 열이 `0.600/0.500/0.400` vs
  `1.000/0.500/0.000` 으로 갈리고 `jnt_01` 이 `vtx_02_a` 에 **0.400** 을 흘린다
  (= 보고된 문제 그대로). 회귀로 고정
- **닫힌 루프** — 6열 링(폭 `1.0/3.0` 교차, 조인트 3개 비균일)에서도 요청 1·2 동일,
  마지막 조인트 → 첫 조인트를 **감아 도는 구간**의 열도 조인트 합 1.0 으로 덮인다
- **반경이 정말 안 쓰인다** — `radius × across` 다섯 조합(`0.0001/0` · `1/0` · `1000/999` ·
  `0.0001/999` · `7.5/0.3`)의 결과 웨이트가 **전부 동일**
- **seed 스냅** — 조인트를 표면에서 2.0 띄워도 결과 동일
- **방어** — 루프 없이 `even=True` 는 조용히 무시하지 않고 명확한 에러
- **행 합** — 9 버텍스 전부 정확히 1.0 (기존 인플루언스가 나머지를 유지)
