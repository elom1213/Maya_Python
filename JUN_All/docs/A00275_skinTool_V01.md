...
| 〃 | 〃 | **Copy Weights** (v01.17~) | **같은 메시 안에서** 버텍스 → 버텍스로 웨이트를 **그대로** 복사. 목표마다 가장 가까운 소스 버텍스의 웨이트 행을 통째로 베낀다. 최근접 기준은 **Surface / Topology / Volume** |
...
3. **붙여넣을 버텍스를 선택** → `PASTE weights onto selected vertices`.
