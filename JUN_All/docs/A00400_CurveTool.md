...
| | **Transform** (v01.18~) | 리스트업한 커브의 **셰이프 자체**를 **각 커브의 피벗 기준**으로 **스케일 · 이동 · 회전**한다. CV 를 전부 골라 툴을 쓴 것과 같은 결과이고 **트랜스폼 채널은 건드리지 않는다**. X/Y/Z 축마다 켜고 끈다. 기본은 **Scale 만**. **v01.19~ 값 아홉 개를 전부 슬라이더로** |
...
3. 각 줄에서 **X / Y / Z 축 체크박스**로 적용할 축을 고르고, **슬라이더를 끌거나**(v01.19~)
   숫자 칸에 값을 친다.
4. **`Apply to Shapes`**. 다시 누르면 한 번 더 걸린다(스케일은 곱해지고, 이동·회전은 더해진다).

```
┌ Shape Transform (about each curve's pivot) ─────────────────┐
│ [v] Scale   [v] Uniform                                     │
│   [v] X  [──────●──────────────]  [ 1.000 ]                 │
│   [v] Y  [──────●──────────────]  [ 1.000 ]                 │
│   [v] Z  [──────●──────────────]  [ 1.000 ]                 │
...
| **슬라이더** (v01.19~) | 값 아홉 개(3줄 × XYZ)를 **전부 끌어서** 정한다. 숫자 칸과 **양방향**으로 맞물린다 |
| 축 체크박스 | **끄면 그 축은 중립값**(스케일 1 / 이동·회전 0) — 아무 것도 안 한 것과 같다. 끈 축은 슬라이더·숫자 칸이 **회색**이 된다 |
| `Uniform` | **Scale 전용**. 한 축을(슬라이더든 숫자든) 정하면 **켜 둔 다른 축**이 따라온다(세 축을 같은 배율로 키우는 것이 대부분이라 기본 켬) |
| `Reset Values` | 칸을 기본값으로(스케일 1 / 이동 0 / 회전 0) + **슬라이더 범위도 기본으로**. **씬은 건드리지 않는다** |

### 슬라이더 범위 (v01.19~)

| 줄 | 슬라이더가 덮는 구간 | 숫자 칸이 받는 범위 | 슬라이더 한 칸 |
|----|----------------------|---------------------|----------------|
| `Scale` | `0 ~ 5` | `-1000 ~ 1000` | 0.01 |
| `Move` | `-10 ~ 10` | `-100000 ~ 100000` | 0.01 |
| `Rotate` | `-180 ~ 180` | `-3600 ~ 3600` | 0.5 |

- 슬라이더는 **자주 쓰는 구간**만 덮는다. 처음부터 전체 범위를 덮으면 한 픽셀이 수십 단위가 되어
  슬라이더가 쓸모없어진다.
- **그보다 큰 값을 숫자 칸에 치면 슬라이더 범위가 그 값까지 늘어난다** — 잘라 버리면 사용자가
  친 값이 조용히 바뀌기 때문이다. 늘어난 범위는 `Reset Values` 로 되돌린다.
- **슬라이더는 값만 정한다.** 씬은 예전처럼 **`Apply to Shapes` 에서만** 바뀐다(끄는 동안 커브가
  실시간으로 변하지 않는다).
...
**슬라이더 34항목 (v01.19)** — 슬라이더 9 · 숫자 칸 9 · 슬라이더→숫자 / 숫자→슬라이더 양방향 ·
기본 범위 3줄 · 숫자 칸의 전체 범위 유지 · **범위 밖 값이 잘리지 않고 범위가 늘어남**(음수 스케일 포함) ·
`Reset` 이 값과 범위를 함께 되돌림 · `Uniform` 이 슬라이더 드래그에도 걸림 · 끈 축은 회색이고 값은 중립 ·
꺼진 줄은 `values()` 가 `None` 이고 `Reset` 해도 깨어나지 않음 · 슬라이더 값으로 실제 적용 · undo 한 스텝.
탭 최소 폭은 **826 그대로**, 최소 높이만 496 → 712(창 최소 높이는 +26px).
