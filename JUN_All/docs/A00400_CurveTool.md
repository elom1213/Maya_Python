...
| | **Controls** (v01.15~) | 컨트롤러 커브 **34종**을 만들고(Parent / Child / World / Origin), **색**을 입히고, 이미 있는 컨트롤의 **셰이프를 교체**한다. `bs_controls` 이식 |
...
**`Load Shape`**(바꿀 컨트롤) → **`Load Replacement`**(모양을 가져올 커브) → **`Replace Shapes`**.

- 타깃의 **이름 · 트랜스폼 · 연결은 그대로** 두고 커브 셰이프만 갈아 끼운다(셰이프 이름은 `<타깃>Shape`).
- replacement 가 **하나면 모든 타깃**에, **타깃과 개수가 같으면 짝끼리**. 그 외에는 아무것도 하지 않고
  이유를 로그에 적는다 — 어느 짝이 맞는지 툴이 추측하면 안 된다.
- **`Mirror Shapes`** 는 `scaleX = -1` 로 뒤집어 넣는다(반대쪽 컨트롤용). 이때는 타깃 자리에 맞추지 않는다.
