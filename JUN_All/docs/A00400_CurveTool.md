...
| **Display**<br>**어떻게 보이는지**를 바꾼다 | **Replace** (v01.16~) | 컨트롤의 **셰이프를 다른 커브 모양으로 교체**한다(이름·트랜스폼·연결은 그대로). 대상과 교체본을 **TSL 두 칸**에 담아 순서대로 짝짓는다. **v01.22~ 대상이 레퍼런스면** 셰이프 대신 **CV 를 하나씩 맞춘다** · **v01.23~ `Resolve Pair from Selection`** 으로 반대쪽 대상을 자동으로 채운다 |
...
- **버전**: `app/config/version.py` (v01.23 — `Display > Replace` 에 **Resolve Pair from Selection**)
...
> **선 굵기 칸은 v01.24 에 없앴다** — `nurbsCurve.lineWidth` 는
> `Display > Shape Edit` 의 `Line Width` 하나가 담당한다(v01.21 에 옛
> `Line Width` 탭이 거기로 합쳐졌다). 만들 때 한 번 정하는 칸과 목록으로 언제든 바꾸는 칸이
> 같은 어트리뷰트를 두고 나뉘어 있을 이유가 없다. 새 컨트롤은 `lineWidth` 를 건드리지 않는다
> (마야 기본 `-1`).
...
**34종 전부 원본과 CV 단위로 일치** · Gear 셰이프 2개 · 이름 규칙 3가지 ·
...
### `Resolve Pair from Selection` (v01.23~) — 반대쪽 대상을 자동으로 채운다

리그 **한쪽을 만들어 두고 반대쪽에 같은 모양을 입히는** 흐름을 위한 버튼이다.
교체본마다 **반대쪽 이름**을 만들어, 그 이름의 커브가 씬에 있으면
왼쪽 `Shapes to replace` 에 짝지어 채운다 — 대상을 손으로 고를 필요가 없다.

1. 모양을 가져올 커브(예: `arm_l_ctl`)를 고른다. 이미 `Replacement` 에 담아 뒀다면 선택은 없어도 된다.
2. **`Resolve Pair from Selection`** — `arm_r_ctl` 이 씬에 있으면 왼쪽에 담긴다.
3. `Mirror Shapes` 를 켜고 **`Replace Shapes`**.

| 대상 | 씬 선택이 있으면 그것, 없으면 `Replacement` 리스트 |
|---|---|
| 토큰 규칙 | **Framework 공용 파일 하나** — `Framework/rules/mirror_tokens.json` ([`MirrorTokenStore`](Framework_mirror_tokens.md)) |
| 이름 매칭 | **경계 매칭** — `sample_lip_l_ctl` 의 `_lip` 을 잘못 집지 않는다 |

> [!warning] ★ 양쪽 리스트를 **짝지어진 것만으로 함께** 다시 채운다
> 짝은 **리스트 순서**로 맺어진다(아래 "짝 짓는 규칙"). 왼쪽만 채우면 짝을 못 찾은 교체본
> 때문에 **뒤가 한 칸씩 밀려 엉뚱한 셰이프가 적용된다.** 그래서 이 버튼은 짝이 맺어진 것만
> 남기고 **양쪽을 함께** 다시 쓴다. 빠진 교체본은 개수와 이름을 로그에 적는다.

**건너뛰는 것** — 전부 사유가 로그에 남는다.

| 경우 | 로그 |
|---|---|
| 좌/우 토큰이 없다(센터 컨트롤) | `has no left/right token` |
| 반대쪽 이름의 **커브**가 씬에 없다 | `is not a curve in the scene` |
| 같은 이름의 노드가 커브가 아니다(조인트 등) | 〃 — 커브 셰이프를 가진 트랜스폼만 본다 |
| 같은 이름의 커브가 여럿 | 첫 번째를 쓰고 `matches N curves` 로 알린다 |
...
### 대상이 레퍼런스일 때 — CV 매칭 (v01.22~) ★

**레퍼런스로 불러온 컨트롤은 셰이프 노드를 지울 수 없다.** 위의 갈아 끼우기는
"기존 셰이프 삭제 → 새 셰이프 붙이기" 라서 레퍼런스 대상에서는
`Cannot delete ... as it has locked or read-only children` 로 막혔다.

v01.22 부터 **대상이 레퍼런스면 셰이프는 그대로 두고 CV 만 옮긴다** —
`crv_to_replace.cv[i]` 를 `crv_replacement.cv[i]` 에 하나씩 맞춘다.
두 커브의 **모양이 원래 같고 CV 개수가 같으면**(= 좌우 미러 쌍) 의도한 모양이 된다.

| `Mirror Shapes` | 대상 CV 가 어디로 가나 |
|---|---|
| **켬** | 대응 CV 의 **월드** 위치를 **X 만 뒤집은**(`-x, y, z`) 자리. 교체본을 **월드 기준 `scaleX = -1`** 한 모양이다 |
| **끔** | 대응 CV 의 **오브젝트** 위치 그대로 |

두 경우 다 **셰이프 갈아 끼우기 경로와 같은 결과**를 낸다(끔 = `matchTransform` 후 freeze 와 같다).

- **켜고 끌 것이 없다** — 대상마다 레퍼런스인지 자동으로 보고 갈린다.
  레퍼런스와 로컬이 **섞인 리스트**도 한 번에 돌아가고, 어느 쪽으로 처리했는지 **로그에 적힌다**.
- **셰이프 개수나 CV 개수가 다르면 그 대상은 아무 것도 쓰지 않고** 사유와 함께 건너뛴다
  (반쪽만 옮겨 모양이 깨지지 않게).
- 트랜스폼 · 이름 · 연결은 그대로고, 변경은 **레퍼런스 edit**(`setAttr ....controlPoints[i]`)로
  저장돼 **씬을 다시 열어도 남는다**. 역시 **undo 한 스텝**.

> **왜 `xform` 으로 쓰나** (mayapy 2024 로 확인) — `cmds.curve -replace`(`Shape Edit` 탭의 방법)는
> 레퍼런스 커브에서 **그 세션에만** 먹고 저장되지 않는다. `setAttr .controlPoints[i]` 는 저장은
> 되지만 히스토리가 살아 있는 커브에서 **델타(트윅)** 로 들어가 값이 두 배가 된다.
> `cmds.xform(cv, objectSpace=True, translation=...)` 만 세 가지(절대 위치 · 저장 · undo)를 다 만족한다.
> 또 **주기(닫힌) 커브는 `cv[i]` 가 spans 개까지만** 유효해 그 위 인덱스로 쓰면 마지막 CV 가
> 덮어써진다 — 앞의 spans 개만 쓰고 이음매는 마야가 따라오게 둔다.

### 검증 (mayapy 2024 + 오프스크린 Qt, 13 + 24항목 통과)

**v01.16 (13항목)** — 같은 개수 → 순서대로 1:1 · 대상이 더 많음 → 앞 2쌍만, 나머지 불변 + 로그 ·
교체본이 더 많음 → 앞 2쌍 · 교체본 1개 → 전부 · Display 하위 탭 구성 ·
`Create > Controls` 에서 교체 위젯이 사라짐 · TSL 두 개 · 씬 선택으로 리스트업 · UI 실행 ·
빈 리스트 경고 · 창 폭.

**v01.22 (24항목)** — 레퍼런스 감지 · Mirror ON 이 대응 CV 의 미러된 월드 위치와 일치(주기 커브
이음매 포함) · 셰이프 노드 생존 · 트랜스폼 불변 · **저장했다 다시 열어도 남는다**(레퍼런스 edit) ·
**undo 한 스텝**으로 전부 복원 · Mirror OFF 의 오브젝트 공간 복사 · **히스토리가 살아 있는**
레퍼런스 커브도 절대 위치(델타 아님) · CV 개수가 다르면 아무 것도 안 씀 + 사유 ·
열린/닫힌 커브가 섞이면 걸러 냄 · 레퍼런스와 로컬이 섞인 리스트 · 빈 리스트/없는 노드/커브 아닌
교체본 방어 · **로컬 대상은 기존 셰이프 교체 그대로**(회귀).
...
3. 각 줄에서 **X / Y / Z 축 체크박스**로 적용할 축을 고르고, **슬라이더를 끌거나**(v01.19~)
   숫자 칸에 값을 친다.
4. `Live` 가 켜져 있으면(기본) **끄는 동안 이미 씬에 반영**된다. `Apply to Shapes` 는 그 모양을
   **확정**하는 버튼이다. `Live` 를 끄면 예전처럼 `Apply` 를 눌러야 걸리고, 다시 누르면 한 번 더
   걸린다(스케일은 곱해지고, 이동·회전은 더해진다).
...
| `Live` (v01.20~) | **기본 켬.** 슬라이더를 끄는 동안 씬의 커브가 바로 따라 변한다(아래 [라이브](#라이브--끄는-동안-커브가-따라-변한다-v0120)) |
| `Reset Values` | 칸을 기본값으로(스케일 1 / 이동 0 / 회전 0) + **슬라이더 범위도 기본으로**. 라이브로 변형 중이었다면 **커브도 드래그 전 모양으로** 되돌린다 |
...
### 라이브 — 끄는 동안 커브가 따라 변한다 (v01.20~)

**`Live` 가 켜져 있으면(기본) 슬라이더를 끄는 그 순간 씬의 커브가 그대로 따라 변한다.**
값을 정해 놓고 `Apply` 를 눌러 확인하는 왕복이 사라진다.

| 상황 | 일어나는 일 |
|------|-------------|
| 슬라이더를 끈다 | 리스트의 커브가 **즉시** 그 값으로 변한다 |
| 왔다 갔다 끈다 | **누적되지 않는다.** 2배로 키웠다 1.5배로 끌면 **원본의 1.5배** |
| 손을 떼거나 조작이 멎는다 | 그때까지의 결과가 **undo 한 항목**으로 기록된다 |
| `Ctrl+Z` | **드래그 한 번 = 되돌리기 한 번**. 드래그 시작 전 모양으로 |
| `Apply to Shapes` | **지금 모양을 확정**만 한다(또 걸지 않는다). 값 칸은 1 / 0 으로 돌아가고, 다음 조작은 그 모양에서 시작 |
| `Reset Values` | **드래그 시작 전 모양으로 되돌린다**(undo 한 스텝) + 값·슬라이더 범위 초기화 |
| 리스트를 바꾼다 | 지금까지의 결과를 확정하고 세션을 닫는다. 커브는 그대로, 값 칸은 1 / 0 부터 다시 |
| 사용자가 `Ctrl+Z` 를 누른다 | 씬이 세션의 가정과 어긋난 것을 알아채고 **세션을 버린다** — 다음 조작은 지금 씬 모양을 새 기준으로 |
| `Live` 를 끈다 | 예전 방식: 값만 정하고 **`Apply to Shapes`** 에서만 씬이 바뀐다(`Apply` 를 또 누르면 곱해진다) |

**어떻게 누적을 막나** — 세션이 시작될 때 **CV 원위치**를 들고 있다가, 값이 바뀔 때마다
**그 원위치에서 다시** 계산해 쓴다(상대 이동이 아니라 절대 위치). 그래서 슬라이더를 아무리
흔들어도 결과는 "원본 × 지금 값" 하나뿐이다.

**undo 규칙** (`A00110_animTool_V02` 의 Stagger Offset 과 같은 방식)
- 끄는 동안의 미리보기는 `undoInfo(stateWithoutFlush=False)` 로 **undo 큐에 안 쌓는다**
  (`state=False` 는 쌓인 히스토리를 통째로 날리므로 쓰지 않는다).
- 조작이 멎으면(0.35초 디바운스 또는 슬라이더에서 손을 뗄 때) **먼저 마지막으로 기록된
  상태로 되돌린 뒤**(기록 없이) 거기서 한 번에 써서 기록한다 — 그래야 `Ctrl+Z` 가
  미리보기 상태가 아니라 **기록된 상태**로 돌아간다(restore-before-commit).

> 커브 200개로 잰 한 틱 비용은 **약 30ms**(1개 0.3ms · 50개 7ms, headless 기준)다.
> 아주 긴 목록에서 뷰포트까지 무거우면 `Live` 를 꺼 두면 된다.
...
**라이브 41항목 (v01.20)** — 끄는 즉시 반영 · **왕복해도 누적 안 됨** · 1 로 돌리면 원래 모양 ·
드래그 여러 틱 + 확정이 **undo 한 번** · redo · 두 번째 드래그가 첫 결과 위에 쌓이고 undo 가 그 사이로 ·
라이브 중 `Apply` 가 **두 번 걸지 않음**(확정 + 값 초기화) · 확정 뒤 다음 드래그는 확정된 모양에서 ·
`Reset` 이 드래그 전 모양으로(그 되돌림도 undo 한 스텝) · `Live` OFF 면 드래그로 안 변하고 `Apply` 는 예전대로 ·
`Live` 를 끄는 순간 화면의 모양이 확정됨 · Move 도 라이브 · 축/줄 체크가 즉시 반영 ·
리스트를 바꾸면 세션이 확정되고 새 커브로 새 세션 · **사용자가 Ctrl+Z 를 누르면 세션을 버림** ·
각 커브의 피벗 사용 · 빈 리스트 · 커브 아닌 항목.
...
탭 최소 폭은 **826 그대로**, 최소 높이만 496 → 734(v01.20 의 `Live` 한 줄 포함, 창 최소 높이는 +48px).
