...
> **V02 v02.18 — Timing > Move: `Offset` 기본값 1**: 창을 열면 Offset 칸에 `1` 이 들어 있다
> (예전엔 비어 있어 `Enter Offset.` 경고부터 났다). 선택 키를 한 프레임씩 미는 용도에 맞췄다.
...
- **Start / End**: 작업할 시간 범위(프레임). **Offset**: 이동량(양수 입력, 부호는 버튼이 결정, **기본값 1** — v02.18~).
