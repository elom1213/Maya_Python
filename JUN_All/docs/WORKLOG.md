...
> [!summary] A00110_V02 **Timing > Move: Start / End 를 비우면 그래프 에디터에서 고른 키만 이동** (v02.16->02.17)
- 요청: 구간(Start/End) 으로 키를 옮기다 보면 **구간 밖으로 나간 키는 Earlier/Later 를 눌러도 더 안 움직인다**. Start/End 없이 그래프 에디터에서 고른 키만 Offset 만큼 옮기고 싶다.
- **Start / End 가 둘 다 비었을 때만** 새 모드로 간다 — 구간 방식은 그대로다. 키 선택이 이동을 따라가므로 반복 클릭해도 계속 밀린다.
- 선택 안 된 이웃 키는 **넘어갈 수 있게** `option="over"` 로 옮긴다(기본 `move` 는 이웃 키에서 멈춰 Offset 이 지켜지지 않는다).
- ★ **`over` 는 도착 프레임에 키가 있어도 거부하지 않는다** — 키를 `19.9999f` 처럼 바로 앞에 밀어 넣는다(mayapy 로 확인). 그래서 **옮기기 전에 겹침을 검사**해 그 커브는 통째로 건너뛰고 `[WARN]` 로 커브 이름을 알린다.
- 검증(mayapy 2024): 여러 커브 선택 키 +3 두 번 연속 이동, 이웃 키 넘기, 겹침 커브만 건너뜀, Ctrl+Z 한 번으로 전부 복원, 선택 키 없음 경고.

---

## 2026-09-22

> [!summary] Framework/TSL **`(Null)` 자리표시를 빨간 글씨로** — A00145 Pair 포함 모든 툴 (A00145 v01.53->01.54)
- 요청: A00145 `Pair` 탭의 TSL 에 `(Null)` 이 보이면 **빨강**으로. 다른 툴의 TSL 도 `(Null)` 이 보이면 같게.
- **공용 TSL 위젯에 넣었다** — `set_items` · `append_unique` 등 채우는 모든 경로에서 자동으로 칠한다. 그래서 A00145 `Pair` 뿐 아니라 TSL 을 쓰는 **75곳 전부**가 같은 색을 받는다(대표 세 툴로 확인).
- 색은 오늘 만든 공용 [[framework-log-level-colors]] 의 **실패 색과 같은 빨강**을 쓴다 — 저장소의 빨강이 한 곳에서 정해지고 **밝은 테마에서도 읽힌다**(테마에 따라 값이 바뀐다).
- ★ **A00145 `Connect > Match` 리스트는 TSL 을 거치지 않는다** — `dst_list.addItems(...)` 로 직접 채운다. 그래서 위젯의 자동 칠이 닿지 않아, 같은 규칙을 **모듈 함수** `mark_null_items()` 로 빼고 그쪽에서 부르게 했다. (그 리스트는 원래 **회색**이었다 — 요청대로 빨강으로 통일.)
- 자리표시 텍스트는 `NULL_ITEM_TEXTS`(기본 `("(Null)",)`)이고 인스턴스마다 `null_texts` 로 바꿀 수 있다 — 다른 표식을 쓰는 툴이 생겨도 색 규칙은 하나다.
- 표식을 **왜 남기는지**를 위젯 주석과 문서에 적었다: 순서로 짝을 맺는 기능에서 빈 자리를 지우면 뒤가 한 칸씩 밀려 **엉뚱한 것끼리 이어지고, 연결은 성공하므로 에러도 안 난다**. 남은 표식이 눈에 띄어야 하는 이유다.
- 검증: mayapy 2024 + 오프스크린 Qt **29항목 통과** — 색(어두운/밝은 테마) · set_items/append_unique/요약 모드 해제 · 다시 채우면 지워짐 · 툴별 표식 교체 · 직접 채운 QListWidget · A00145 Pair 실제 매칭 결과 · Connect > Match(툴팁 유지) · 대표 세 툴(A00275 · A00440 · A00380). #A00145 #Framework
...
- ★ **파라미터 등분 ≠ 길이 등분.** CV 간격이 불규칙한 커브에서는 간격이 불규칙해지는데, 그것이 요청(POCI 와 같은 자리)의 뜻이다. 문서에 못 박고, 길이로 고르게 놓는 기능은 A00400 Joints 탭이라고 가리켰다. 테스트에도 "불규칙 커브에서 간격이 서로 다르다" 를 한 항목으로 넣었다.
- 닫힌(주기) 커브는 parameter 0 과 1 이 같은 점이라 **마지막 조인트가 첫 조인트에 겹친다**. 자리를 임의로 비켜 놓으면 규칙이 깨지므로 **겹쳐 두고 로그로 알린다**.
- 체인은 이 툴의 기존 생성 기능과 같은 관례(`joint -e -zso -oj xyz -sao yup`, 마지막은 `oj none`). 체크박스를 끄면 각 조인트 전에 선택을 비워 **루트**로 만든다(조준 없음 — 조준할 자식이 없다).
- 기존 `Joints to Crv`(CV/EP 기준)는 손대지 않았다. 새 칸은 같은 탭 아래 `By Count` 그룹으로 추가.
- 검증: mayapy 2024 + 오프스크린 Qt **46항목 통과** — 비율 계산(3->0/0.5/1.0, 1 이하 거부) · **POCI 좌표 일치**(기본/부모 변형/degree 1) · 파라미터 등분 확인 · 체인 부모·orient · 분리 모드 · 여러 커브 · 커브 아닌 항목 · 닫힌 커브 · undo 한 스텝 · UI(스핀박스 최소 2, 기본 3, 체크박스 기본 켬, 실제 클릭 두 모드, 빈 리스트 경고, 기존 버튼 회귀). **마야 GUI 실기 확인은 사용자 몫**. #A00060
...
- 새 파일 **`Framework/core/log_levels.py`** 가 표식->색 대응을 갖고(Qt·maya 비의존), 공용 로그창 `JUN_mod_log_qt_v01` 이 그것으로 칠한다. **툴 코드는 한 줄도 안 바뀐다** — A00480 도 `self._log("[WARN] ...")` 그대로 색이 붙는다. 47툴이 함께 받는다. 끄려면 `colorize_levels=False`.
- ★ **한 색으로는 안 된다 — 테마에 밝은 것도 있다.** `*_light.qss` 는 배경이 `#eef…` 라서 어두운 배경용 노랑(`#ffcc33`)이 거의 안 보인다. 그래서 색을 **(어두운 배경용, 밝은 배경용) 한 쌍**으로 갖는다.
- ★ **qss 는 `QPalette` 를 바꾸지 않는다** — "위젯 배경색을 읽어 밝기를 재자" 는 처음 생각이 통하지 않았다(테마를 입혀도 팔레트는 기본값). 그래서 `ThemeManager` 가 **마지막으로 불러온 테마 이름을 기억**하게 하고(`current_theme()` · `is_dark_theme()`), 판정은 이름의 `_light` 하나로 한다.
- ★ **색깔 줄 다음 평문 줄이 그 색을 물려받는다** — `QTextEdit` 커서가 글자 포맷을 이어 쓰기 때문이다. 그냥 두면 **첫 `[WARN]` 이후 모든 줄이 노랗게** 찍힌다. 평문을 넣기 전과 색깔 줄을 넣은 뒤에 `QTextCharFormat()` 을 다시 걸어 비운다. 테스트로 고정했다.
- 두 경로 모두에서 칠한다 — 툴 절반은 `appendPlainText`, 절반은 `append` 로 평문을 넣는다. 다만 `append` 에 `<` 가 들어 있으면 **툴이 손으로 쓴 HTML** 로 보고 통과시킨다(A00300 · A00410 · A00430 의 색이 그대로 이긴다). 그 세 툴을 공용 표로 옮기는 것은 하지 않았다 — 요청 범위가 아니고, 지금도 서로 부딪치지 않는다.
- 들여쓰기가 살아야 한다 — 색깔 줄은 HTML 이라 QTextDocument 가 앞 공백을 하나로 줄인다. `white-space: pre-wrap` 을 span 에 줘서 `  - [OK] ...` 가 평문 줄과 어긋나지 않게 했다. `Copy`/`toPlainText()` 는 **평문 그대로**다.
- 등록되지 않은 대괄호는 지나친다 — `[Set_v001]` · `[1/3]`(정규식이 글자만 받는다). 대소문자는 안 가린다(A00480 이 쓰는 `[Info]` 도 파랑).
- 검증: mayapy 2024 오프스크린 **65항목 통과** — 규칙·위젯 36(각 색 · 색 물려받지 않음 · `<node>` 가 글자로 · Copy 평문 · append 경로 · 툴 HTML 통과 · 끄기 · 밝은 테마 · Expand 창) · A00480 실기 10(테마 slate_dark · 툴이 실제로 내는 메시지) · 대표 툴 19(A00470 · A00440 · A00110_V02 · A00380 · A00400 + 손색칠 3툴 회귀). 마야 GUI 에서 눈으로 보는 확인은 사용자 몫. #Framework #A00480
...
- ★ **왼쪽만 채우면 안 된다.** 짝은 **리스트 순서**로 맺어지므로(`replace_shapes`), 짝을 못 찾은 교체본이 오른쪽에 남아 있으면 **뒤가 한 칸씩 밀려 엉뚱한 셰이프가 적용**된다. 그래서 짝이 맺어진 것만 남기고 **양쪽 리스트를 함께** 다시 쓴다. 빠진 것은 개수와 이름을 로그에.
- 대상은 **씬 선택이 있으면 그것, 없으면 이미 담긴 `Replacement` 리스트** (버튼 이름의 "from Selection" 과 이미 담아 둔 경우를 둘 다 살린다).
- 건너뛰는 것은 전부 사유를 로그에 — 토큰 없음(센터) · 반대쪽이 씬에 없음 · 같은 이름이 **커브가 아님**(조인트 등, 커브 셰이프를 가진 트랜스폼만 본다) · 자기 자신으로 미러됨 · 동명 커브가 여럿(첫 번째를 쓰고 알린다).
- TSL 에는 전체 경로가 아니라 **마야가 주는 고유한 최소 이름**을 넣는다(`cmds.ls(node)[0]`) — 경로를 그대로 넣으면 목록이 읽기 어렵고, 짧은 이름만 쓰면 동명 노드에서 엉뚱한 것이 잡힌다.
- 검증: mayapy 오프스크린 **16항목 통과** — 정상 짝 · 경계 매칭(`sample_lip_l` -> `sample_lip_r`) · 반대쪽 없는 것 제외 · 토큰 없는 센터 제외 · 커브 아닌 동명 노드 제외 · 좌우 개수 일치 · 선택 없을 때 리스트 사용 · 둘 다 비었을 때 경고 · 짝지은 결과로 `Replace Shapes` 가 도는지. 마야 GUI 확인은 아직. #A00400

> [!summary] A00400 **Replace — 레퍼런스 대상은 셰이프 대신 CV 를 맞춘다** (v01.21->01.22)
- 요청: `Display > Replace` 에서 `Shapes to replace` 의 커브가 **레퍼런스**면 셰이프 노드를 지울 수 없어 에러가 났다. 그럴 때는 `crv_to_replace.cv[i]` 를 `crv_replacement.cv[i]` 에 맞춰 달라 — `Mirror Shapes` 를 켜면 교체본을 **월드 기준 scaleX = -1** 한 자리로, 꺼도 CV 매칭은 하도록.
- 대상마다 레퍼런스인지 보고 갈린다(화면에 켤 것 없음). 레퍼런스면 셰이프는 그대로 두고 CV 만 옮기고, 아니면 기존 셰이프 교체 그대로. **섞인 리스트도 한 번에** 돌고 어느 쪽으로 처리했는지 로그에 적는다. 셰이프/CV 개수가 다르면 **그 대상은 한 점도 쓰지 않고** 사유와 함께 건너뛴다.
- ★ **`cmds.curve -replace` 는 레퍼런스 커브에서 그 세션에만 먹는다** — 레퍼런스 edit 로 기록되지 않아 **저장했다 다시 열면 원래 모양**이다. 처음엔 `Shape Edit` 탭과 같은 이 방법을 쓰려다 mayapy 로 걸렀다. `setAttr .controlPoints[i]` 는 저장은 되지만 **히스토리가 살아 있는 커브에서 델타(트윅)** 로 들어가 값이 두 배가 된다. 셋 중 `cmds.xform(cv, objectSpace=True, translation=...)` 만 절대 위치 · 저장 · undo 를 다 만족한다.
- ★ **주기(닫힌) 커브는 컴포넌트 `cv[i]` 가 spans 개까지만 유효하다.** MFn 은 `spans + degree` 개로 세는데(뒤 degree 개는 앞의 복사본) 그 인덱스로 `xform` 을 쓰면 **조용히 마지막 CV 로 클램프돼** 엉뚱한 CV 가 망가진다(원 11개에 cv[8..10] → cv[7]). 앞의 spans 개만 쓰면 이음매는 마야가 따라온다.
- ★ **`cvPositions()` 가 준 `MPoint` 를 `MFnNurbsCurve` 밖으로 들고 나오면 안 된다.** 값이 원래 자리로 되돌아가 **CV 가 한 점도 안 움직이는데 로그는 성공**이라고 찍혔다 — 씬을 저장했다 다시 연 다음에만 재현돼 한참 찾았다. 읽는 즉시 plain tuple 로 복사한다(`MPlug.asMObject` 와 같은 수명 함정).
- 검증: mayapy 2024 **24항목 통과** — 미러 CV 위치(이음매 포함) · 셰이프 생존 · 트랜스폼 불변 · 저장/재오픈 후 유지 · undo 한 스텝 · 오브젝트 공간 복사 · 히스토리 커브 · 개수 불일치 시 무작업 · 열린/닫힌 혼합 차단 · 섞인 리스트 · 방어 4종 · 로컬 경로 회귀. #A00400

> [!summary] A00380 **창 가로를 A00400_CurveTool 과 같게** (v01.14->01.16, 한 번 틀리고 고침)
- 요청: A00380_MeshTool 의 가로를 A00400_CurveTool 과 같게.
- ★ **오프스크린에서 잰 최소 폭을 그대로 박았다가 "너무 넓다" 는 말을 들었다(v01.15).** mayapy 오프스크린으로 재면 A00400 의 최소 폭이 **1176** 으로 나와 그 값을 썼는데, **마야에서는 훨씬 좁게 열린다** — 오프스크린 플랫폼의 기본 폰트가 마야 UI 폰트보다 넓어 글자 기반 최소 폭이 전부 부풀려지기 때문이다. 테마(qss)를 입혀 재도 이 차이는 남는다.
- ★ **[[offscreen-size-needs-theme]] 메모에 이미 같은 사고(A00290_V02 "가로가 너무 길다")가 적혀 있었는데 반복했다.** 메모를 한 단계 더 날카롭게 고쳤다 — "테마를 입히고 재라" 에 더해 **"테마를 입혀도 오프스크린 px > 마야 px, 잰 최소 폭을 창 크기로 박지 말 것"**.
- 고침(v01.16): 가로는 **A00400 의 `resize()` 값(360)을 그대로 요청**한다. 실제 폭은 두 툴 모두 자기 레이아웃 최소까지만 벌어지므로 **같은 규칙으로 열린다**. 세로 860 은 그대로.
- 같은 조건(테마 yellow_dark, 둘 다 같은 오프스크린)에서 A00380 최소 폭(748)이 A00400(1176)보다 작으므로 **A00380 이 A00400 보다 넓어질 일은 없다**.
- 재는 도중 A00400 이 다른 세션의 편집 중간 상태라 import 가 깨져(`write_cv_points` 미정의), **HEAD 판을 임시 루트로 꺼내** 같은 조건에서 쟀다(그쪽 작업 파일은 건드리지 않았다).
- 검증: 요청 폭이 두 툴 같은지 · 직전(1176)보다 좁아졌는지 · A00400 보다 넓지 않은지 · 세 탭이 잘리지 않는지 · MeshDoctor 진단 회귀 — 오프스크린 8항목 통과. **마야에서 실제로 보이는 폭 확인은 사용자 몫**이다. #A00380

> [!summary] A00370 **런처 버튼을 현행 툴로 정리** — 옛 툴 가리키던 5개 (v01.06->01.07)
- 요청: 예전 툴이 버튼으로 등록돼 있으면 삭제하고, 현행 툴을 쓸 수 있게 버튼을 갱신.
- ★ **"더 새 폴더가 있다" 가 곧 "옛 툴" 은 아니다.** 번호 접두사로 묶어 보면 후속 폴더가 있는 버튼이 넷이었지만, 그것만으로는 판단할 수 없어 **각 툴의 문서·메모가 무엇을 현행으로 적는지**로 갈랐다 — A00110 메모 "이후 작업은 V02 에서 한다" · A00050 메모 "V01 은 그대로 둔다"(V02 가 PySide 이식판) · docs/README 가 `A00060_jointTool_V02` 를 "탭 재분류 전 · 보존" 으로 표기 · A00290 V02 가 탭 재분류판. 넷 다 옛 폴더의 마지막 실질 변경이 **Framework 일괄 작업(09-17)** 이고, 새 폴더만 09-18/09-21 에 기능이 들어갔다.
- 정리: `meshDoctor`(A00300) **삭제** — 어제 A00380 의 MeshDoctor 탭으로 이식했고 같은 프로파일에 MeshTool 버튼이 이미 있다. `uvTool`·`BSTool`·`AnimTool`·`jointTool_V02` 는 현행 버전 폴더로 **교체**. 버튼 이름에도 버전을 붙여(`uvTool_V02`) 어느 쪽이 열리는지 화면에서 보이게 했다.
- ★ **이 종류의 썩음은 에러를 내지 않는다** — 옛 폴더를 지우지 않는 저장소라 링크가 안 깨지고, 사용자만 조용히 옛 툴을 쓰게 된다. 그래서 문서에 "툴에 새 버전을 만들면 런처 버튼도 함께 옮긴다" 를 경고로 박았다.
- ※ 작업 중 내 편집 스크립트가 `Mesh.json` 을 항목마다 디스크에서 새로 읽어 **마지막 수정만 남는** 버그가 있었다. 검증에서 잡혀(옛 경로 2개가 그대로) 파일당 한 번만 읽도록 고쳐 다시 적용했다.
- 검증: 런처 자신의 `tool_launcher.launch()` 로 **버튼 14개를 전부 실행** — 실패 0, 옛 툴을 가리키는 버튼 0(mayapy 오프스크린). 프로파일은 git 추적 대상이라 이 정리가 모든 PC 의 기본값이 된다. #A00370
- 그 뒤 `RIG` 프로파일에서 **skinMigrate 버튼이 빠졌다**(런처 UI 에서 편집된 것으로 보인다). 프로파일은 git 추적 대상이라 그대로 따라가고, 문서의 기본 프로파일 표도 맞췄다.

> [!summary] A00380 **MeshDoctor 탭** — A00300_meshDoctor 를 통째로 이식 + 아이콘 새로 그림 (v01.13->01.14)
- 요청: `A00300_meshDoctor` 를 `A00380_MeshTool` 의 새 **MeshDoctor** 탭으로 모두 이식하고, 아이콘도 "메시를 수정·편집·진단하는 툴" 로 느껴지게. (작업 중 추가 요청: **MeshDoctor 를 첫 번째 탭으로**)
- 진단 로직 `mesh_scan.py` · `mesh_fix.py` · `report.py` 3개를 **한 줄도 고치지 않고** 옮겼다 — 바꾼 것은 패키지 경로 2곳과 리포트 머리말뿐. 외부 참조가 `Framework.core.maya_shape` 와 자기 패키지밖에 없어 이식이 깨끗했다.
- ★ **이식이 충실한지 결과로 확인했다** — 같은 씬(정상 구 · 큐브 · 떠돌이 정점 메시 · 평면 · 병합으로 흐트러뜨린 실린더)에서 옛 `MeshScanner` 와 새 `MeshScanner` 의 `scan_nodes()` 결과를 JSON 으로 덤프해 비교, **완전히 동일**.
- 탭에서 바꾼 것 넷: ① 대상 리스트를 손수 만든 `QListWidget` 대신 **공용 TSL 위젯**으로(이 툴이 이미 쓰고 있다 · UUID 보관) ② 상세 리포트를 **탭 안 리포트 뷰**로 분리(공용 로그창은 높이 110 의 한 줄짜리 상태 표시줄이라 수십 줄 리포트를 담으면 둘 다 못 읽는다) ③ 진단 직후 **가장 심한 메시를 자동으로 펼친다** ④ 표의 행을 누르면 **씬에서도 그 메시가 선택**된다.
- 탭 순서는 **MeshDoctor → Peak → Match**. 작업 순서가 그렇다 — 무엇이 잘못됐는지 보고 나서 고친다. 탭 인덱스에 기대는 코드가 없는 것을 확인하고 옮겼다.
- ★ **창 크기**: 테마를 입히고 재니 최소가 **748 x 805** 였다. 폭을 끄는 것은 MeshDoctor(457)가 아니라 **Match 탭(722)** 이다 — 예전 값 `380 x 520` 은 진작부터 최소보다 작아 Qt 가 늘려 주던 값이었다. 실측값 `760 x 860` 으로 적었다.
- 아이콘: 요소 셋으로 제한했다 — **누운 와이어프레임 격자**(대상) · **버텍스를 노말 방향으로 끌어올리는 화살표**(Peak/Match = 편집) · **오른쪽 아래 십자 배지**(MeshDoctor = 진단). 32px 로 구워 실제 크기에서 읽히는지 확인했다. PNG 는 `dev/build_icons.py` 와 같은 Qt 방식으로 이 아이콘만 다시 구웠다(전 툴 png 를 건드리지 않으려고).
- 검증: mayapy 오프스크린 **20항목 통과**(탭 순서 · 요약 표 · 행 클릭 상세+씬 선택 · 자동 펼침 · 리포트가 A00380 의 `0020_out/` 에 · TSL 리스트 우선 · 창 크기 · 탭 전환). 스캐너 동일성 비교 통과.
- ※ 마야 GUI 실기 확인(셸프 드롭 → 진단 → Fixes 버튼)은 아직. 옛 `A00300_meshDoctor` 는 관례대로 **지우지 않고 남겼다**(문서 머리에 이사 안내). #A00380 #A00300
...
- 계획에 없던 것 하나를 더했다 — **`install.bat` 이 `update.py` 의 종료 코드를 보게** 했다. 그러지 않으면 새로 넣은 "실패하면 exit 1" 이 아무 효과가 없다(배치가 그대로 등록 단계로 넘어간다).
- 릴리즈 반영: 툴 11개를 다시 빌드해 드롭 파일 10개가 교체됐다(그 외 파일은 어제 릴리즈와 같아 변화 없음). 릴리즈본 드롭 파일 **10/10** 이 같은 대역 테스트를 통과하고, 클린 경로 import **11/11** · Qt 툴 6개 `run()` 도 그대로 통과한다.
- 마지막으로 **새로 클론한 PC 를 흉내 내** `update.py` 를 돌려 통과(exit 0) — 새로 받는 쪽은 처음부터 정상이다.
- 커밋 4개 (저장소가 둘이라 나뉜다) — `Maya_Python` `8ac6e25`(드롭 55개 + 문서 · 계획서 · 메모) / `Maya_Tool_Release` `80f3730`(userSetup) · `cfd3ca1`(update + install.bat) · `637425e`(툴 10개 재빌드). **릴리즈 저장소 제목은 전부 ASCII 로 썼다.**
- 남은 것 : ① 마야 GUI 에서 구분선 있는 셸프에 드롭해 보기 ② `setup_app_dir.py` 가 `C:/Users/<사용자>/Documents/maya/scripts` 를 하드코딩한다 — Documents 가 OneDrive 로 리디렉션된 PC 에서는 **C 와 같은 종류의 조용한 실패**가 난다(`os.path.expanduser("~")` 기준으로 잡으면 된다). 이번 범위 밖이라 두었다.
...
- Sort 버튼에 툴팁을 달았다(무엇을 기준으로 정렬하는지 · 짝이 리스트 순서라는 것). 우측 TSL 툴팁의 "use Up / Down" 도 "use Sort or Up / Down" 으로.
- ★ 정렬은 공용 위젯의 `_set_records(sorted(...))` 라 **UUID 가 유지**되고, 리스트 모델 신호가 울려 `update_match_pairs()` 가 돌면서 **미리보기 세션이 버려진다** — 옛 짝으로 Apply 되는 사고가 안 난다(리스트를 바꿀 때와 같은 경로).
- 오프스크린 Qt 10항목 통과: 버튼·툴팁 · 좌우 이름순 정렬 · 세션 폐기 · 정렬 뒤 리네임해도 UUID 로 노드 추적 · 짝 미리보기가 k 번째끼리 · 정렬한 짝으로 Apply 가 실제로 돈다(5.0 이동). 마야 GUI 에서는 아직 안 눌러 봄. #A00380

> [!summary] **release_builder_QT** — 목록 **검색(Filter)** · 공용 로그창 · 아이콘 (v01.01->01.02)
- 요청 셋: ① 툴 목록에 filter 검색 ② 툴 아이콘 ③ 공용 로그창 위젯 사용.
- **Filter**: 공용 `JUN_mod_filter_qt_v01` 을 붙였다(부분 일치 · 대소문자 무시 · 공백 단어 AND · `Number: 보이는수 / 전체수`). 툴이 **66개**라 눈으로 찾는 게 일이었다.
- ★ **체크와 필터가 따로 논다** — Qt 는 숨긴 항목의 체크를 유지한다. 그래서 `Select All` / `Clear` 는 **보이는 것에만** 걸고(필터를 쓰는 이유가 그것), 릴리즈 때는 **가려진 채 체크된 개수를 로그로 알린 뒤 함께 내보낸다**(체크는 명시적 의사표시라 없던 일로 만들지 않되 모르고 나가지도 않게 — A00145 Attribute 탭과 같은 판단). `Refresh` 는 필터 글자를 지킨 채 다시 먹인다.
- **로그창**을 공용 `JUN_mod_log_qt_v01` 로(Expand / Shrink / Clear / Copy). 릴리즈 로그가 길어 Expand 가 실익. ★ 공용 위젯의 `append()` 는 **HTML 을 해석**하므로 경로와 `=` 구분선이 있는 이 로그는 `appendPlainText` 로 쓴다.
- **아이콘** 신규 — svg / png(32) / **다중 크기 ico**(16~256). 열린 상자 + 위로 나가는 화살표. 각 크기를 **SVG 에서 직접 렌더**하고 **가장 큰 프레임을 base** 로 저장(한 장 축소는 `availableSizes` 가 한 크기만 본다). ★ 터미널 실행이라 **`QApplication` 앞에서 AppUserModelID** 를 걸어야 작업 표시줄이 파이썬 아이콘을 안 쓴다 → 새 `app/config/app_meta.py`.
- 오프스크린 Qt **30항목** 통과. 새 문서 `docs/release_builder_QT.md`. 실제 릴리즈 복사는 돌려 보지 않았다(목적지 경로가 외장 드라이브). #dev
...
- 수정: **`MFnMesh.setVertexNormals` / `setFaceVertexNormals` 한두 번**으로 쓴다 → +114 MB · 1.8초. 한 버텍스의 face-vertex 가 전부 같은 값이면 버텍스 단위로 써서 **공유 노멀을 쪼개지 않는다**(28→56 방지).
- ★ **`cmds.dgdirty` 가 반드시 필요하다** — API 로 쓰면 노멀이 쪼개진 사실이 **skinCluster 출력까지 가지 않아** 출력이 옛 공유 구조를 들고 값이 이웃과 뒤섞인다(40개 중 26개가 틀렸다). 명령으로 쓸 때 명령이 대신 해 주던 일.
- ★ **undo 를 되살렸다** — 쓸 버텍스 범위(`vtx[a:b]`)에만 `polyNormalPerVertex -freezeNormal` 을 **먼저 한 번** 걸면(이미 잠긴 노멀엔 값이 안 바뀌는 no-op) 그 시점 노멀이 undo 레코드에 담긴다. 뒤이은 API 쓰기는 큐에 없지만 Ctrl+Z 가 그 명령을 되돌리며 **옛 노멀이 통째로 복구**된다(1,560/1,560).
- ★ **참조된 셰이프에는 아예 안 쓴다** — 편집 폭발은 API 로 써도 같다(마야가 참조 메시 노멀 변경을 face-vertex 마다 편집으로 보존). 개수를 적은 경고를 남기고 건너뛴다.
- 디포머 출력 당김을 **4회 → 1회**(v01.28 과 같음). `getNormals()` 배열은 **그 메시 데이터가 살아 있는 동안만** 유효해서, 놓고 나면 뒤이은 setAttr 이 버퍼를 재활용해 값이 밀린다 — 데이터를 함께 붙들었다.
- 새 옵션 `Keep locked vertex normals`(기본 ON). mayapy 2024 검증: 잠김/안 잠김/부분 잠김 · undo · Snap to rest · blendShape · parallel EM + 스킨 2개 · 359k face-vertex 시간·메모리 · 참조 리그 편집 수 · 오프스크린 UI. 마야 GUI 에서는 아직 안 눌러 봄. #A00275

> [!summary] A00400 **Display > Transform** — 슬라이더를 끄는 동안 **씬의 커브가 라이브로 따라 변한다** (v01.19->01.20)
- 요청: 셰이프 조절 슬라이더를 움직이면 씬의 커브가 그에 맞게 실시간으로 변형되게.
- 새 코어 `ShapeTransformSession` — 세션 시작 때 **CV 원위치**를 잡아 두고 값이 바뀔 때마다 **원위치에서 다시** 계산해 절대 위치로 쓴다. 그래서 슬라이더를 왕복해도 **누적되지 않는다**(2배 → 1.5배로 끌면 원본의 1.5배).
- undo 는 A00110 Stagger Offset 방식 그대로: 드래그 중 미리보기는 `undoInfo(stateWithoutFlush=False)` 로 **큐에 안 쌓고**, 멎으면(0.35초 디바운스 · 슬라이더 릴리스) **마지막 기록 상태로 되돌린 뒤** 한 번에 써서 **한 항목**으로 기록한다(restore-before-commit) → **드래그 한 번 = Ctrl+Z 한 번**.
- ★ 두 가지를 실수로 잡아냈다: ① 세션을 닫을 때 **위젯 값이 아니라 세션이 실제로 건 값(`applied`)** 을 확정해야 한다 — 닫는 계기가 "값이 방금 바뀐 것" 이라 위젯은 이미 다음 값이다. ② 리스트 교체는 **TSL 모델 신호**로 즉시 닫아야 한다. `_live_update` 안에서 늦게 닫으면 그 순간 값 칸을 0 으로 돌리며 **사용자가 방금 끈 값이 사라진다**.
- `Live` 체크박스(기본 ON, 끄면 예전처럼 Apply 에서만). 라이브 중 `Apply` 는 **또 걸지 않고 지금 모양을 확정**(값은 1/0 으로), `Reset` 은 **드래그 전 모양으로 되돌린다**(undo 한 스텝). 사용자가 Ctrl+Z 를 누르면 씬이 어긋난 것을 탐침으로 알아채고 세션을 버린다.
- mayapy 2024 오프스크린 Qt **41항목**(+ v01.19 의 34항목 회귀) 통과. 한 틱 비용 200커브 **30ms**(1커브 0.3ms). 창 최소 높이 +22px. 마야 GUI 에서는 아직 안 눌러 봄. #A00400

> [!summary] A00145 **Match > Keep Children in Place** — 팔로워만 옮기고 그 아래 오브젝트는 있던 자리에 (v01.52->01.53)
- 요청: Targets 에 맞춰 움직이는 Followers 의 **자식들은 지금 자리에 그대로** 있어야 할 때가 있다. 체크박스로, **기본은 해제**.
- Mirror 탭(v01.41)에 이미 같은 기능이 있어 `mirror_manager` 에서 **공용 `app/core/keep_children.py`** 로 떼어 냈다(capture -> 부모 이동 -> restore, 잠기거나 연결된 채널은 건너뛰고 사유 보고). mirror 쪽은 같은 이름으로 위임만 남겨 동작 불변 — 두 탭의 기본값만 다르다(Mirror ON / Match OFF).
- Match 규칙: 직계 자식만(손자는 로컬이 그대로라 저절로 제자리) · 읽기는 **아무것도 옮기기 전에 전부** · 켜져 있으면 **부모 -> 자식 순서**로 매칭(팔로워끼리 부모/자식이면 자식을 먼저 맞췄다가 부모가 다시 끌고 간다) · 자식 쪽 팔로워는 보존 대상에서 제외 · 손계가 바뀌면 scale 채널까지 검사. 로그 플래그에 `K` 가 붙고 지킨 개수를 적는다.
- mayapy 2024 27항목 + 오프스크린 Qt UI 10항목 통과(자식 보존 · 끄면 따라감 · 손자 · 자식 팔로워 · 회전/스케일 · 음수 스케일 · 잠긴 자식 · 컨스트레인트 노드 · 1<-n · undo · **Mirror 회귀**). 마야 GUI 에서는 아직 안 눌러 봄. #A00145

> [!summary] A00400 **Display > Transform** — 값 칸 아홉 개를 전부 **슬라이더**로 끌어서 조절 (v01.18->01.19)
- 요청: 커브 셰이프 조절용 SpinBox 를 **모두 슬라이더로** 조절할 수 있게.
- 3열 그리드(축 체크 + 숫자 칸)를 **축마다 한 줄**(`[v] X [───●───] [ 1.000 ]`)로 바꿨다. 아홉 개 값 전부 슬라이더와 숫자 칸이 **양방향**으로 맞물린다. 새 `_AxisSlider` 위젯(툴 안, `QSlider` 는 정수라 step 으로 나눠 정수 칸으로 쓴다).
- ★ **슬라이더 범위는 자주 쓰는 구간**(스케일 0~5 · 이동 ±10 · 회전 ±180)이고 숫자 칸은 예전 그대로 전체 범위(±1000 / ±100000 / ±3600). 전체 범위를 슬라이더로 덮으면 한 픽셀이 수십 단위라 못 쓰고, 그렇다고 잘라내면 사용자가 친 값이 조용히 바뀐다 → **범위 밖 값을 치면 슬라이더 범위가 그 값까지 늘어난다.** `Reset Values` 가 값과 범위를 함께 되돌린다.
- 슬라이더는 값만 정한다 — 씬은 예전처럼 `Apply to Shapes` 에서만 바뀐다(드래그 중 라이브 변형 아님). `Uniform` 은 슬라이더 드래그에도 걸리고, 끈 축은 슬라이더·숫자 칸이 회색이 된다.
- mayapy 2024 오프스크린 Qt **34항목** 통과(양방향 동기 · 범위 확장 · Reset · Uniform · 꺼진 줄/축 · 슬라이더 값으로 실제 적용 · undo 한 스텝). 탭 최소 폭은 **826 그대로**, 최소 높이만 496->712 로 창 최소 높이 +26px. 마야 GUI 에서는 아직 안 눌러 봄. #A00400

> [!summary] A00130 V02 **Orient & Place** — 오른팔 전체를 **왼팔의 Behavior 미러**로, 그리고 **한 조인트 = 한 규칙** (v02.26->02.27)
- 요청: `helper_clavicle_r` 하위 헬퍼 본은 전부 왼쪽을 Behavior 미러한 방향이어야 한다. 예) 왼쪽 `helper_upperarm_l` 이 `+X` forward · `+Y`→월드 `-Y` · `+Z`→월드 `-Z` 면, 오른쪽은 `-X` forward · `+Y`→월드 `+Y` · `+Z`→월드 `+Z`. 그리고 규칙을 둘씩 받는 본(A3 + A3(late) 등)을 하나만 받게.
- ★ 원인: **오른팔에 규칙이 둘이었고 둘의 답이 달랐다.** 이른 A3 미러가 넣은 방향을 `arm_r` A2 가 덮는데, A2 는 `+Z` 를 **미러된 폴 타깃** 쪽으로 둔다 — 폴 타깃은 x 만 뒤집히므로 `+Z` 가 **왼쪽과 같은 쪽**을 본다(미러와 1.98 차이, 실측). 사용자가 본 "오른쪽 `+Y`·`+Z` 가 왼쪽과 같은 방향" 이 정확히 이것이다.
- 수정: 오른팔은 **늦은 미러 하나**가 전부 갖는다. `clavicle_r` A1 · `arm_r` A2 · `hand_l -> hand_r` 늦은 미러를 **없애고**, 팔 미러(`clavicle_l -> clavicle_r` 하위 전부)를 `"stage": "late"` 로 — 왼팔·왼손이 다 정렬된 뒤 통째로 복사한다. 결과 오른팔 23개가 미러와 **5.7e-14** 일치.
- ★ **위치만 먼저 미러한다**(`"position_stage": "early"`) — 오른쪽 폴 타깃이 A2 앞에서 팔에 물리고 **그때 포즈에서 거리를 굽기** 때문. 덕분에 좌우 `poleDistance` 가 같다(3.6742 = 3.6742).
- 표: **한 조인트에 행 하나**. 나중 단계가 최종 권한이고, 위치를 먼저 옮기는 준비 단계는 규칙이 아니라 노트(`prepared by A3 position`)로. 84 조인트 = 84 행, 규칙 없는 것은 `helper_root` 하나.
- 미러가 **채널이 구동되는 조인트를 건너뛴다** — `helper_polTgt_arm_r` 은 제 컨스트레인트가 몰고 있어 두 번째 실행부터 반쯤 옮겨지거나 에러만 났다. 로그에 `1 driven, left to their own rule`.
- mayapy 2024 **46항목 통과**: 미러 일치 · 사용자 시나리오 재현 · 한 조인트 한 행 · **옛 규칙과 두 벌(`A:`/`B:`) 대조해 오른팔 밖 60개는 불변** · 멱등 · 미러 끄기 · 폴 타깃 좌우 대칭. 마야 GUI 에서는 아직 안 눌러 봄. #A00130

> [!summary] A00110 V02 **Mirror Key > Scale** — 스케일 채널도 반대쪽으로, **미러 없이 값 그대로** (v02.15->02.16)
- 요청: Mirror Key 탭에 `Scale` 체크박스를 더해 스케일도 복붙되게. **스케일은 그대로 변화 없이** 복사.
- `Channels` 에 `Scale`(기본 OFF) 추가. 켜면 소스의 `scaleX/Y/Z` 가 타겟에 **부호도 안 뒤집고** 그대로 들어간다 — 크기는 좌우가 같아야 하는 값이고, 반사 행렬을 거쳐 다시 분해하면 축 순서에 따라 **음수 스케일이나 셰어**가 섞인다. 그래서 `Behavior` 를 켜든 끄든(월드 반사 모드에서도) 스케일 경로는 하나다.
- 구간 미러 · 현재 프레임 미러 둘 다 적용. 셋 다 끄면 `Enable Translate, Rotate and/or Scale.` 로 막고, **Scale 만 켜고도** 실행된다. 잠긴 스케일 채널은 다른 채널과 똑같이 건너뛴다.
- `mirror_key_manager` 에 `S_AXES` · `_scale_values` · `do_scale` 인자(기본 False — 기존 호출부는 그대로). mayapy 2024 헤드리스 **30항목** 통과: 두 모드 값 일치 · 음수 스케일 보존 · 스위치 OFF 면 스케일 키 0 · 스케일만 애니된 소스도 `Source keys` 가 잡음 · Bake · 잠긴 채널 · 현재 프레임 per-channel 키잉 · 센터 self-mirror · undo 한 스텝 · 오프스크린 Qt 배선(창 최소 폭 586 그대로). 마야 GUI 에서는 아직 안 눌러 봄. #A00110
...
- 코어 — `maya_sets.sets_of / parent_sets / shapes_of / is_default_set / is_render_set` + `set_manager.run_find_sets`. 옵션 4개: 셰이프까지 볼지(기본 ON — **컴포넌트 세트와 셰이딩 그룹은 셰이프에 붙는다**) · 셰이딩 그룹 · 부모 세트(A ∈ B ∈ C) · 마야 기본 세트.
- ★ **행 클릭이 멤버가 아니라 세트를 선택**하도록 공용 TSL 에 `select_no_expand` 를 새로 뒀다(`cmds.select` 는 세트를 펼친다). `Find` 탭에서만 켠다 — `Edit` 탭 Split 은 "씬 선택 == A 의 멤버" 를 사고 감지에 쓰고 있어 예전 동작 그대로 둔다. 호출부 133곳이 전부 키워드 인자라 기본값 OFF 로 영향 없음.
- ★ 실측 두 가지: `listSets` 는 면·버텍스를 함께 담은 세트를 **두 번** 돌려준다(중복 제거) · **`listRelatives(<세트>, shapes=True)` 는 에러가 아니라 세트를 펼쳐 멤버의 셰이프를 준다** → DAG 인지 먼저 확인하고 묻는다.
- 옵션 라벨이 길어 창 최소 폭이 531 → 615 로 늘어나던 것을 라벨을 줄여 되돌렸다(Find 탭 469 < Edit 493). mayapy 2024 헤드리스 **41항목** 통과(코어 + 오프스크린 Qt). 마야 GUI 에서는 아직 안 눌러 봄. #A00440
