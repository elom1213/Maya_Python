...
> [!summary] 신규 툴 A00220 BackupTool — 주기적 자동 백업(standalone PySide) + A00110 animTool Follow 탭(target 추종 베이크) + A00210 FileManager Lineage 탭(파일 브랜치/병합 그래프)
- **A00210_FileManager**: **Lineage 탭 신설**(v01.02) — 여러 리비전 폴더에 흩어진 마야 파일들의
  **브랜치/병합 관계(DAG)** 를 직접 기록하고 `git log --graph` 스타일 **색상 레인 트리**로 본다.
  **인터랙티브 캔버스**(`QGraphicsView`): 노드 드래그 이동 + **Connect Mode** 로 노드→노드 선 긋기로
  ...
  드래그 위치 저장). **Planned**("제작 예정") placeholder 노드(점선·반투명). 스캔으로 노드 추가 시
  project-relative key 로 기존 record/썸네일 링크. 에셋별 이름 그래프 `<store_dir>/lineage/<name>.json`
