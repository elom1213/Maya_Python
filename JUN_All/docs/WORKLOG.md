...
> [!summary] A00145 RigConnect Constrain 탭에 Skin Weight to Constraint 추가(v01.02, 접이식) + A00220 BackupTool Settings 접이식화·창 슬림(v01.03)·스핀박스 화살표 PNG 복구·백업 log.txt(v01.02) + A00110 animTool Follow 탭 강화(Maintain Offset · 1<-n · Get Current) + A00110/A00120 뷰포트 프리즈(refresh suspend 누수) 수정 & Force Refresh 버튼 + A00210 FileManager Lineage 노드 로그 동기화(v01.08)·노드/연결 삭제 & 다중선택(v01.09) + Framework dark 테마 Win10 트리 행 가독성
...
- **A00210_FileManager Lineage 노드/연결 삭제 + 다중선택**(v01.09): Lineage 탭에서 **연결선(엣지)도
  ...
  **빈 캔버스 드래그 = 러버밴드 다중선택**(`ContainsItemShape`), Connect Mode 중엔 러버밴드 off(선 긋기 집중)·
  해제 시 복원. **Delete Node** 버튼도 선택 노드를 **일괄(팝업 없이)** 삭제로 변경. #A00210_FileManager
