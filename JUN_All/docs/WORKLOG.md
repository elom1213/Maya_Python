...
- `Set Up` 상자를 세로로 바꿔 두 리스트 **아래 가운데**에 버튼을 두었다. Match 탭의 `Swap`
  (`Targets` <-> `Followers`) 과 **같은 이름 · 같은 동작**이라 손에 익은 자리에서 벗어나지 않는다.
...
- headless(mayapy 2024) 확인 — 창이 뜨고, `(Null)` 이 낀 3행짜리 두 리스트가 자리 순서
  그대로 맞바뀐다.
