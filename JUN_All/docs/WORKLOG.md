...
> [!summary] A00380 **창 가로를 A00400_CurveTool 과 같게** (v01.14->01.15)
- 요청: A00380_MeshTool 의 가로를 A00400_CurveTool 과 같게.
- ★ **`resize()` 에 적힌 숫자가 곧 창 폭이 아니다.** A00400 코드에는 `resize(360, 620)` 이라고 적혀 있지만 그 툴의 **최소 폭이 1176** 이라 실제로는 늘 1176 으로 열린다 — 코드의 360 을 그대로 베꼈으면 두 창의 폭이 전혀 달랐을 것이다. 테마를 입히고 **실측**해서 1176 을 얻었다.
- A00380 은 `1176 x 860`. 자기 최소가 748 x 805 이라 1176 은 Qt 에 깎이지 않는다. 세로는 건드리지 않았다.
- 재는 도중 A00400 이 **다른 세션의 편집 중간 상태**라 import 가 깨졌다(`write_cv_points` 미정의). 1176 은 그 전에 잰 값이고, 그쪽 UI diff 가 툴팁 문구뿐이라 폭에 영향이 없음을 확인했다.
- 검증: 목표 폭으로 열리는지 · Qt 가 깎지 않는지 · 세 탭 모두 잘리지 않는지 · MeshDoctor 진단 회귀 — 오프스크린 9항목 통과. #A00380
