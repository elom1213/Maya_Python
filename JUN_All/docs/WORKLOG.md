...
- **구현(`app/ui/main_window.py`)**: 메뉴 바 우측 코너(`setCornerWidget(..., Qt.TopRightCorner)`)에
  체크형 `Pin` 버튼 배치. `toggle_always_on_top()` 이 `Qt.WindowStaysOnTopHint` 를 켜고/끄고,
  플래그 변경 시 창이 숨는 Qt 특성을 피하려 **`show()` 재호출**. ON 이면 라벨 `Pinned`, 로그에도 기록.
- **검증**: `py_compile` 통과(실기 확인은 마야에서 `run(True)` 리로드로 예정). 문서
