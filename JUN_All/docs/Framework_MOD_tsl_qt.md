...
- **애초에 노드 이름이 될 수 없는 모양이면 마야에 묻지도 않는다**(`_looks_like_node`). 노드 이름은
  글자나 `_` 로 시작하고(롱네임이면 `|`) 그 뒤로 글자/숫자/`_` 와 `|:.[]` 만 오므로, 공백이나 `@` 가
  들어간 텍스트는 노드일 수 없다. 헛도는 마야 호출이 사라지고, `cmds.ls` 가 그런 문자열에 **예외를
  던지는** 것도 피한다. `A00145_RigConnect` 의 캐시 항목(`@cache pCube1`)이 이 경로로 리스트에 얹힌다
  — 리스트에는 오브젝트처럼 보이지만 씬에서 찾지 않는 항목이다.
