...
담는 방법은 두 가지다. 편한 쪽을 쓰면 된다.

**(A) 한쪽씩 몰아서 — Add Source / Add Target** (권장)

1. 소스 메시 위의 가이드 커브들을 **원하는 순서대로** 골라 **Add Source**.
2. 타깃 메시 위의 대응 커브들을 **같은 순서로** 골라 **Add Target**.
3. 행 순서대로 짝이 맺힌다.

담는 규칙은 **"비어 있는 칸을 위에서 아래로 먼저 채우고, 남으면 그만큼 행을 새로 만든다"** 이다.
그래서 개수가 안 맞아도 나중에 모자란 쪽만 더 담으면 **빈 칸부터 이어서** 채워진다.

- 한쪽만 찬 행은 `< empty >` 로 보이고 **Info** 칸에 `waiting for source` / `waiting for target` 이 뜬다.
- 그 상태로 **Wrap** 하면 **몇 번째 행의 어느 쪽이 비었는지 알리고 그 행만 건너뛴다**(조용히 넘어가지 않는다).
- 타깃이 소스보다 많으면 짝 없는 행이 생기고 로그로 경고한다. 반쪽 행은 **Swap** 으로 반대 칸에 옮길 수 있다.
- 커브가 아닌 선택은 무시하고 개수를 알린다.

**(B) 쌍으로 한 번에 — Add Curve Pair**
...
- 점 하나로 못 박고 싶은 곳(입꼬리·눈꼬리 등)은 버텍스나 로케이터를 2개 골라 **Add Point Pair**.
- **거꾸로 넣었으면(타깃 커브를 먼저 골랐으면) 그 행을 선택하고 Swap** — 소스/타깃이 맞바뀐다.
...
- 잘못 넣은 행은 골라서 **Remove**, 잠깐 빼고 싶으면 **On** 체크를 끈다.
...
| 증상 | 원인 | 대처 |
|------|------|------|
| `Guide row N has no target` 로그가 뜨고 그 행이 빠진다 | Add Source / Add Target 중 한쪽만 담았다 | 모자란 쪽을 골라 **Add Source / Add Target** (빈 칸부터 채워진다) |
