...
1. 소스 메시 위의 가이드를 **원하는 순서대로** 골라 **Add Source**.
2. 타깃 메시 위의 대응 가이드를 **같은 순서로** 골라 **Add Target**.
3. 행 순서대로 짝이 맺힌다.
...
| 고른 것 | 가이드 종류 | 비고 |
|---------|-------------|------|
| NURBS 커브 | **curve** | 샘플 N 개로 쪼개져 대응이 된다 |
| 메시 **버텍스** `.vtx[]` | **point** | 입꼬리·눈꼬리처럼 점으로 못 박는 자리 |
| 커브 **CV** `.cv[]` | point | |
| 로케이터 · 조인트 | point | 위치만 쓴다 |
| 메시 오브젝트 · 엣지 · 페이스 | **받지 않는다** | 아래 설명 |

> 메시를 통째로 고른 경우를 **일부러 막는다.** 그대로 받으면 피벗 한 점이 조용히 가이드로 들어가
> 결과가 이상해지는데, 나중에 원인을 찾기가 어렵다. 몇 개를 무시했는지 로그에 남는다.
...
- 한쪽은 커브, 다른 쪽은 버텍스인 행은 **`curve / point mismatch`** 로 표시되고 건너뛴다.
  커브는 샘플 N 개, 점은 1 개라 짝을 지을 수 없다.

#### Order — 버텍스를 고른 순서대로 담기

**커브(오브젝트)는 고른 순서 그대로** 담기지만, **버텍스·CV 같은 컴포넌트는 Maya 가 인덱스 순서로**
돌려준다. 그래서 버텍스를 여러 개 한 번에 담으면 소스와 타깃의 짝이 의도와 다르게 맺힐 수 있다.

**Order** 를 켜면 Maya 의 *Track Selection Order* 를 켜서 **고른 순서**를 그대로 쓴다.

- 켠 **그 순간부터** 기록되므로, 켜고 나서 **다시 골라야** 한다.
- 꺼져 있는데 컴포넌트를 2개 이상 담으면 인덱스 순서로 들어갔다고 로그로 알린다.
- 커브만 쓸 거라면 켤 필요가 없다.
- 창을 닫으면 우리가 켠 경우에만 원래 설정으로 되돌린다(다른 툴 창이 같이 쓰고 있으면 유지).
...
- **목록에서 고르면 씬에서도 선택된다.** 어느 **칸**을 눌렀는지가 무엇을 고를지 정한다 —
  **Source 칸** → 소스만, **Target 칸** → 타깃만, **Info 칸** → 양쪽. 어떤 커브·버텍스가 담겼는지
  바로 확인할 때 쓴다. **On / Flip 은 체크박스 칸이라 씬 선택을 건드리지 않는다**(체크하려다 선택이
  바뀌면 곧바로 다음 **Add Source** 가 엉뚱한 것을 담게 되므로).
...
| 메시를 골랐는데 `Nothing usable selected` | 메시 **오브젝트**는 가이드로 못 쓴다 | 그 위의 **버텍스**를 고르거나 커브를 그린다 |
| 버텍스 여러 개를 담았는데 짝이 뒤죽박죽 | 컴포넌트는 **인덱스 순서**로 들어온다 | **Order** 를 켜고 **다시 골라** 담는다 |
| `curve / point mismatch` | 한쪽은 커브, 다른 쪽은 버텍스 | 양쪽을 같은 종류로 맞춘다(커브↔커브, 점↔점) |
...
- **컴포넌트의 선택 순서는 "선택 이벤트" 단위로만 기록된다.** Track Selection Order 를 켜 두어도
  `cmds.select([vtx70, vtx10, vtx40])` 처럼 **한 번에 리스트로** 고르면 `ls(orderedSelection)` 이
  **인덱스 순서**를 준다. 하나씩 클릭(= `select(add=True)` 를 차례로)해야 순서가 잡힌다. 실제 사용자
  조작은 하나씩 찍는 것이라 문제가 없지만, **헤드리스 테스트에서는 `add=True` 로 나눠 골라야** 한다.
  오브젝트(커브)는 한 번에 리스트로 골라도 순서가 유지된다.
