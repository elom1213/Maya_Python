...
예를 들어 **`No Upstream`** 은 *무엇이 밀어 주고 있는지* 없는 것만, **`No Downstream`** 은
*내가 무엇을 밀고 있는지* 없는 것만 남긴다 — 노드망의 **시작점·끝점**을 찾을 때 쓴다.
...
| **No Upstream** (v01.04~) | **들어오는 연결이 없다** — 나를 구동하는 것이 없다. 체인의 **맨 위**를 찾을 때 |
| **No Downstream** (v01.04~) | **나가는 연결이 없다** — 내가 구동하는 것이 없다. 체인의 **맨 끝**을 찾을 때 |

세 규칙 모두 **같은 것을 무시한다**(`select_rules.IGNORED_TYPES`) — 머티리얼 배정 ·
디스플레이 레이어 · 평범한 오브젝트 셋 멤버십. 부모-자식도 DG 연결이 아니라 세지 않는다.

> **`No Upstream` + `No Downstream` 을 같이 고르면** 양쪽 다 없는 것만 남는다. `Standalone` 과
> 비슷해 보이지만 같지는 않다 — **`Standalone` 은 히스토리까지** 보므로 더 엄격하다
> (예: `polyCube` 히스토리가 달린 메시는 연결이 없어도 `Standalone` 에서는 빠진다).
