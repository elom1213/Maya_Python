...
- One window for the animator's repetitive work: key copy/paste (with axis reversal, and **fanning one source out to every target**), **L/R controller key mirroring**, 6-axis pose keys, Offset & Hold, baking (including Smart bake), **Follow (target-match bake)**, and automatic Graph Editor framing (frames to current ± margin on selection).
