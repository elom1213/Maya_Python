...
| `A00290_BSTool` | blendShape 편집 툴. **Maya 기본 Shape Editor를 대체**하는 탭(전체 타겟 목록 + 타겟별 Edit 토글 + 라이브 웨이트 동기화, **타겟 범위/다중 선택으로 값 동시 조절**, **키·애니메이션 레이어가 걸린 타겟도 그대로 편집** — 값이 즉시 반영되고 Auto Keyframe 이면 현재 프레임의 키로 기록, 제스처 단위 Undo), Base Shape 편집, 프레임 단위 셰이프 복사 |
