...
def JUN_cmd_copyName(str_selTool_tsl_base, str_selTool_tsl_tgt):
    str_allItemList_base = cmds.textScrollList( str_selTool_tsl_base, q=True, allItems=True);
    str_allItemList_tgt  = cmds.textScrollList( str_selTool_tsl_tgt , q=True, allItems=True);
    ...
    for i in range(0, int_num_base):
        ...
        cmds.rename(str_allItemList_tgt[i], str_allItemList_base[i]);
...
def PY_JUN_makeUI_copyNameUI ():
    ...
    cmds.window( str_winName, bgc=color_mainDark, title="Copy Name Tool V01" );
    ...
    cmds.button( "name_btn_copyName", 
                 label='Copy Name', 
                 bgc=color_btn, 
                 command='JUN_cmd_copyName(\"JUN_name_toolSelBase_cpyName_tsl\", \"JUN_name_toolSelTgt_cpyName_tsl\")');
