# last Update date 26 01 28
# Python Script by Ji Hun Park

# Control Rig Tool V01.07
...
# 25 11 30    v01.06 : create code for pose objects matching
#                      create code for matching fingers zro node
# 26 01 28    v01.07 : set pole object for expected direction (leg to z+, arm to z-)
...
class JUN_cage():
    ...
    def __init__(self):
        ...
        self.MSN_tkn_C01_Fingers_zro_L = "C01_Fingers_zro_L_meta_X"
        self.MSN_tkn_C01_Fingers_zro_R = "C01_Fingers_zro_R_meta_X"
        ...
        self.MSN_tkn_C04_pose_obects = "C04_pose_obects"

        self.MSN_tkn_A01_helper_arm_l = "A01_helper_arm_l"
        self.MSN_tkn_A02_helper_arm_r = "A02_helper_arm_r"
        self.MSN_tkn_A03_helper_leg_l = "A03_helper_leg_l"
        self.MSN_tkn_A04_helper_leg_r = "A04_helper_leg_r"
        ...
        #===================================================================
        # string for attribute (open)

        self.attr_Con = "Con"
        self.attr_Follow = "Follow"

        # string for attribute (close)
        #===================================================================
...
class JUN_matcher_v02():
    ...
    def set_pos_for_given_axi_only(self, obj_given, axi = "x", moving_dir = "x+" , moving_dist = 50):
        """
        Move object only on given axis.

        obj         : target object name
        axi         : 'x', 'y', or 'z'
        moving_dir  : 'x+', 'x-', 'y+', 'y-', 'z+', 'z-'
        moving_dist : distance to move
        space       : 'world' or 'local'
        """

        if not cmds.objExists(obj_given):
            raise RuntimeError(f"Object does not exist: {obj_given}")
        ...
        pos_ori_world = cmds.xform(obj_given, q=True, worldSpace = True, t=True)
        ...
        cmds.xform(obj_given, worldSpace = True, t=pos_ori_world)
    ...
    def match(self, orient_to_joint=False, jntOrd="xyz", secAxOri="yup", is_leg=False, is_spine__=False, set_ik=False, pole_obj=None, helper_objs=None, tgt_given=None):
        ...
        if is_leg:
            ...
            cmds.joint(member_tgts[-3], edit=True, orientJoint="xzy", secondaryAxisOrient="yup", children=True, zeroScaleOrient=True)
            ...
            cmds.xform(pole_obj[0], worldSpace = False, t= [0, 0, 0])
            self.set_pos_for_given_axi_only(pole_obj[0], axi = "z", moving_dir ="z+")
            posPole_local = cmds.xform(pole_obj[0], q=True, worldSpace = False, t=True)
            posPole_local[1] = 0
            cmds.xform(pole_obj[0], worldSpace = False, t=posPole_local)
...
def get_connected_nodes(node, attrs):
    ...
    for attr in attrs:
        ...
        cons = cmds.listConnections(full_attr, s=True, d=True) or []
...
def JUN_CR_setUp_pose_objects(cage_glo):
    pos_objs = cage_glo.get_pos_objs()
    pos_attr = cage_glo.get_pos_attr()

    for i in range(0, len(pos_objs)):
        node_connected = get_connected_nodes(pos_objs[i], pos_attr)
        node_to_flw = node_connected[cage_glo.get_pos_attr_flw()]
        node_to_con = node_connected[cage_glo.get_pos_attr_con()]

        JUN_MATCH_twoObjects(node_to_flw, pos_objs[i], 1, 1, 1, 1)
        cmds.parentConstraint(node_to_con, pos_objs[i], mo=True)
...
def PY_JUN_makeUI_ContrlRigTool ():
    ...
    # window
    cmds.window( str_winName, bgc=color_mainDark, title="Control Rig Tool V01.07" );
    ...
    cmds.button( "name_btn_CR_connect", 
                 h = win_height/24,
                 label='Set up pose objects', 
                 bgc=color_btn, 
                 command= lambda *argv : JUN_CR_setUp_pose_objects(JUN_cage_glo));
...
def JUN_PY_ControlRigTool_V01_07():
