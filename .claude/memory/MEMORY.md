...
- [A00130 Orient & Place](wip-a00130-orient.md) — 규칙 A1/A2/A3 가 결국 **한 계산**(aim + up 힌트) · 폴 타깃 4개는 **위치까지** 놓아서 단계 이름을 바꿨다. **부모를 돌리면 후손이 전부 딸려 움직인다**(자식만 되돌리면 또 물림) · **forward 를 맞추면 up 은 직교화만큼 기운다** · 폴 타깃 4개는 **거리 고정**으로 문다(배수 json 값은 `d=(n-1)*|v|` 로 환산) (v02.16)
- [A00130 Pair & Constrain](wip-a00130-pair-constrain.md) — 세트 1:1 매칭 + `Con` -> parentConstraint. **parentConstraint 는 드라이버가 다르면 조용히 타깃을 늘린다** · `cmds.sets(q=True)` 는 멤버 순서를 보장하지 않는다 · **`maintainOffset` 기본 off 가 앞 단계(Pair)의 회전을 덮었다** (v02.17)
- [A00130 IK axis](wip-a00130-ik-axis.md) — 폴을 보는 축은 **`twist` 로만** 고쳐진다(`jointOrient` 는 솔버가 보정, `rotate` 는 덮어쓴다) · **`twist` 는 레퍼런스에서 유지** (v02.14)
- [A00060 Pole Target](wip-a00060-pole-target.md) — `A'=A+n·v` 는 **pointConstraint 하나**로 환원된다 · **컨스트레인트 가중치는 `setAttr` 이 음수를 거부하고 연결은 통과** · `Create Selected` 는 **체인 멤버를 건너뛴다**(자기 자신을 모는 컨스트레인트 = 사이클) · **거리 고정은 `n=1+d/|v|` 만 계산해 먹인다**(컨스트레인트는 그대로) · v03.07 **Slide**(양 끝 사이로 미끄러뜨리기 — 슬라이드를 `n` 에 섞으면 `n>1` 에서 부호가 뒤집힌다 · v03.08 배율 0.1 은 **배선된 나눗수를 읽어** 예전 배선을 알아본다) (v03.08)
...
- [A00440 SetTool](wip-a00440-settool.md) — 컴포넌트 세트 집합연산(∪∩∖ + Split) + **`Create` 탭**(오브젝트마다 `<이름>_Set`). 이름 정규화가 전부, **`cmds.select(세트)` 는 멤버를 펼쳐 선택**한다 · **마야는 이름의 앞 숫자를 어디서나 버린다** (v01.01)
...
- [A00410 SecondaryMotion](wip-a00410-secondarymotion.md) — **신규**: FK 체인 관성(KawaiiPhysics식)을 키로 굽기. nucleus 안 쓰는 이유, 전 구간 30ms 재계산→override 레이어, addKeys 74배, **joint `local=R*JO` / transform `local=RA*R`**, 부모를 앞 체인 노드로 가정하면 오프셋 그룹에서 깨짐. v01.02 Bone Chain/Root + 출력 레지스트리(`outputs.py`) 확장 지점. v01.08 **Rotate Axis**(고른 축에만 기록 — 축을 바꾸면 프리뷰 레이어를 지워야 한다) · v01.05 **Graph**(파라미터 배수 커브 — fps 보정은 곱하기 전에) · v01.04 **Loop**(구간을 사이클로 프리롤 — 마지막 바퀴 목표열이 원본과 같아야 하고, **위치 오차는 회전 오차보다 관대해 보인다**)
- [A00400 Smooth tab](wip-a00400-smooth-tab.md) — 고른 CV 실시간 Smooth/Rough. `smoothCurve` 는 **음수를 무시하고 주기 커브에서 실패**, 소프트셀렉션은 `getRichSelection()`(빈 선택이면 **예외**). **닫힌 커브는 CV 를 감아 복사한 임시 열린 커브로 우회** — `.cv[i]` 개수(spans)와 `cvPositions()` 개수(spans+degree)가 다르고, `curve -r` 은 `periodic`+`knot` 을 같이 줘야 한다. **선택을 지우는 명령이 셋** — `smoothCurve` · `curve`(생성) · `delete` (v01.09)
...
- [A00275 Copy Weights](wip-a00275-copy-weights.md) — **신규**: 같은 메시 안에서 버텍스→버텍스 웨이트 복사. **탐색 범위를 Expand Bind 에서 베끼면 못 간다**(집합 안으로 가두는 건 falloff 얘기), volume 최근접은 전수 비교 말고 **균일 격자**(20배, 답은 동일). **Blend**(0~1)는 볼록결합이라 정규화가 안 깨진다 (v01.18)
