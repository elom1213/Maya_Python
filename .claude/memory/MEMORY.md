...
- [A00410 SecondaryMotion](wip-a00410-secondarymotion.md) — **신규**: FK 체인 관성(KawaiiPhysics식)을 키로 굽기. nucleus 안 쓰는 이유, 전 구간 30ms 재계산→override 레이어, addKeys 74배, **joint `local=R*JO` / transform `local=RA*R`**, 부모를 앞 체인 노드로 가정하면 오프셋 그룹에서 깨짐. v01.02 Bone Chain/Root + 출력 레지스트리(`outputs.py`) 확장 지점. v01.05 **Graph**(파라미터 배수 커브 — fps 보정은 곱하기 전에) · v01.04 **Loop**(구간을 사이클로 프리롤 — 마지막 바퀴 목표열이 원본과 같아야 하고, **위치 오차는 회전 오차보다 관대해 보인다**)
