...
- [A00060 Pole Target](wip-a00060-pole-target.md) — `A'=A+n·v` 는 **pointConstraint 하나**로 환원된다 · **컨스트레인트 가중치는 `setAttr` 이 음수를 거부하고 연결은 통과** · `Create Selected` 는 **체인 멤버를 건너뛴다**(자기 자신을 모는 컨스트레인트 = 사이클) · **거리 고정은 `n=1+d/|v|` 만 계산해 먹인다**(컨스트레인트는 그대로) · v03.07 **Slide**(양 끝 사이로 미끄러뜨리기 — 슬라이드를 `n` 에 섞으면 `n>1` 에서 부호가 뒤집힌다) (v03.07)
