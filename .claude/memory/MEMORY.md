...
- [A00400 Smooth tab](wip-a00400-smooth-tab.md) — 고른 CV 실시간 Smooth/Rough. `smoothCurve` 는 **음수를 무시하고 주기 커브에서 실패**, 소프트셀렉션은 `getRichSelection()`(빈 선택이면 **예외**). **닫힌 커브는 CV 를 감아 복사한 임시 열린 커브로 우회** — `.cv[i]` 개수(spans)와 `cvPositions()` 개수(spans+degree)가 다르고, `curve -r` 은 `periodic`+`knot` 을 같이 줘야 한다 (v01.08)
