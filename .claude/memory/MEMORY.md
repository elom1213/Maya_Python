...
- Framework 위젯 — [log level colors](framework-log-level-colors.md)([WARN] 노랑 · [OK] 초록은 `Framework/core/log_levels` 공용 규칙 · 색은 밝은/어두운 테마 한 쌍 · QTextEdit 는 앞 줄 색을 물려받는다) · [log](framework-log-widget.md)(Expand/Shrink/Clear/Copy, **47툴 전부 교체 완료** · 내부는 QTextEdit · Shrink 는 창 높이까지) · [expand](framework-expand-widget.md)(복제 말고 이동) · [filter](framework-filter-widget.md)(v2 트리 모드 · 여러 열 `tree_columns`) · [mirror tokens](framework-mirror-tokens.md)(공용 json, 경계 매칭) · [falloff curve](framework-falloff-curve-widget.md) · [progress popup](framework-progress-widget.md)(마지막 값 스로틀 금지) · [timeRange](framework-timerange-widget.md)
...
- A00050 — [uvTool V02](wip-a00050-uvtool-v02.md)(V01 PySide 이식 · 규칙 위반을 사유와 함께 로그+선택 · rename 은 before->after 와 실패 사유, v02.00 · 바꿀 세트 이름을 화면에서 입력, 칸 하나가 Catch 의 규칙도 정한다, v02.01 · UV Sets 표 + `[wrong_name]` 빨강, **A00380 으로 이식 예정**(표 파일+코어 그대로), v02.02 · Delete UV Sets, 첫(기본) 세트는 못 지움, v02.03 · **A00380 UV Sets 탭으로 이식 완료** — 두 툴의 코어·표 파일은 같은 내용 유지)
- A00060 — [Curve By Count](wip-a00060-curve-by-count.md)(개수로 커브에 조인트 · POCI turnOnPercentage 0~1 등분 = pointOnCurve = API kWorld · **길이 등분이 아니다** · 닫힌 커브는 끝이 겹친다, v03.12) · [Aim Root mode](wip-a00060-aim-root-mode.md)(Chain/Root, 분기는 첫 자식 · X 조준은 자식이 X 축 위일 때만 위치 보존 → 움직인 자식 원위치, v03.11) · [V03 tab reorg](wip-a00060-v03-tab-reorg.md) · [IK Edit](wip-a00060-ik-edit.md)(폴 벡터 역산) · [Pole Target](wip-a00060-pole-target.md)(pointConstraint 하나, Slide, v03.08) · [world-space joint pos](wip-a00060-world-space-joint-pos.md)
...
- A00310 — [Type/Token/Rules](wip-a00310-searchtool-rules.md)(공유 Objects 리스트 · 규칙 레지스트리 · Standalone/No Upstream/No Downstream, v01.04) · A00330 — [Set Rename](wip-a00330-set-rename.md) · [Token tab](wip-a00330-token-tab.md)(Rename 하위 탭, 칸 수 자유 Custom/Numbering + Profile, 마야는 `01_a` 를 `_a` 로, 칸 폭 80px, v01.08) · [Quick Rename > Insert](wip-a00330-insert-tab.md)(TSL 이름 n 번째 자리 삽입, 0 앞 · -1 끝, 라이브 미리보기 + Apply, 깊은 노드부터 rename, v01.09)
...
- A00400 — [CurveTool](wip-a00400-curvetool.md) · [Smooth](wip-a00400-smooth-tab.md)(닫힌 커브 우회, v01.09) · [Points to Curve](wip-a00400-points-to-curve.md) · [Wrap](wip-a00400-curve-wrap.md) · [Joints](wip-a00400-curve-joints.md)(v01.14 NURBS 서피스 U/V 한 줄) · [Controls](wip-a00400-controls-tab.md)(bs_controls 이식, v01.15 · Thickness 칸 삭제 — 선 굵기는 Shape Edit 하나, v01.24) · Combine(v01.11, 쉐입 복사 합치기 — [instance trap](parent-shape-add-is-instance.md)) · [Shape Edit](wip-a00400-shape-transform.md)(옛 Transform · **Line Width 탭 흡수 + 개명, v01.21** — 셰이프 CV 를 피벗 기준 scale/move/rotate + 선 굵기, 트랜스폼 채널 불변, v01.18 · 값 9개 전부 슬라이더 + 범위 밖 값이면 슬라이더 범위를 넓힌다, v01.19 · 라이브 세션 = CV 원위치에서 매번 재계산 + 드래그 한 번에 undo 한 번, v01.20) · **Replace > Resolve Pair from Selection**(교체본의 반대쪽 커브를 공용 미러 토큰으로 찾아 좌측에 — **양쪽 리스트를 짝지어진 것만으로 함께** 다시 쓴다, 짝은 순서로 맺어지므로, v01.23)
...
- A00440 — [SetTool](wip-a00440-settool.md)(v01.01) · [Sets x Objects](wip-a00440-sets-x-objects.md)(Create 의 "전부를 담은 세트 하나" 모드 · Edit 의 Objects 리스트 + Add Objects to Sets — 세트 하나면 전부, 여럿이면 행 순서 1:1, v01.05) · [Find tab](wip-a00440-find-tab.md)(속한 세트 역조회 · `listSets` 는 트랜스폼/셰이프/컴포넌트가 다르고 중복을 준다 · 공용 TSL `select_no_expand`, v01.04) · A00450 — [ManipulatorTool](wip-a00450-manipulatortool.md) · A00460 — [FK & IK](wip-a00460-fk-ik.md)(Skip End Joints = FK 체인 끝 n 개 생략, v01.07)
