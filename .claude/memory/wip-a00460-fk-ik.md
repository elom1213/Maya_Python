...
**④ Skip End Joints (v01.07, 2026-09-18, FK 전용)** — 정수 칸(기본 0). 체인 끝 n 개 조인트는 스택을
만들지 않는다. Bone Root 는 조인트의 **높이**(가장 깊은 잎까지 거리, 잎=0) < n 이면 그 아래 전부 건너뛴다
(`_height` 메모이즈, 가장 긴 가지 기준이라 남긴 조인트의 부모는 안 빠진다 — 가지마다 끝 n 개).
Bone Chain 은 리스트 마지막 n 개. IK 에서는 무시하고 UI 칸도 끈다. 결과 dict 에 `skipped` 추가.
