...
description: 학습 노트는 JUN_Study repo, 6자리 도메인 대역 번호(간격 100)로 정렬한다
metadata:
  node_type: memory
  type: project
---
...
**파일명**: `001000_영문주제.md` — **6자리** 번호 + 영문 주제명. 한글 파일명 금지.
**맨 앞 자리는 비워 둔다**(현재 전부 `0`). 첫 대역 앞이나 새 상위 구분이 필요할 때
뒤 문서를 밀지 않고 넣기 위한 여유다. 2026-08-21 에 5자리 → 6자리로 확장했다.

**도메인 대역**: 001000 모델링/지오메트리 · 002000 스킨/웨이트 · 003000 리깅/컴포넌트(mgear) ·
004000 페이셜/MetaHuman(FACS) · 005000 애님/베이크 · 006000 시뮬/피직스 · 007000 그래픽스 수학(RBF) ·
008000 스크립팅/API · 009000 DCC 외(언리얼·후디니·블렌더).

- 대역 안에서 **간격 100**. 사이에 끼울 때는 중간값(`003150` → `003125` → `003112` …)으로
