...
A00240_PathTool — Help 메뉴 + **Shrink**(v01.10): 탭·메뉴 바를 감추고 **350 x 155** —
A00220_BackupTool 이 줄었을 때와 **같은 크기**(green_mid 오프스크린 실측, [[wip-a00220-shrink]]).
**v01.13**: 처음엔 요청을 "가로 같게, 세로 절반(78)" 으로 만들었는데 사용자가 "너무 작다, A00220 과 같게" 로 바로잡았다.
78px 용 여백 2px 조정은 걷어냈다. 트리는 줄 높이에 맞춰 커진다(155px 에서 아이콘 20x24, 선 4px).
...
- **v01.12 피드백**: 무한 반복(단계마다 0.5초, 한 바퀴 1.5초 — 완성 직후 되감으면 완성된 트리가 한순간만 보임) · 맨 밑 한 쌍 제거(5행) · 연결선+아이콘 테두리 2px(아이콘 10x12). 머무는 구간엔 다시 그리지 않는다.
- Shrink 켤 때마다 처음부터, 끄면 정지. 색은 테마 글자색(공룡과 같은 방식).
