...
`A00410_SecondaryMotion` (2026-09-10, v01.08, 헤드리스 검증 완료, **마야 실기 UI 테스트 대기**)
...
- **v01.08 `Rotate Axis` (X/Y/Z 체크박스)**: 결과를 고른 회전 축에만 기록한다.
  솔버·포즈 재구성은 **그대로 3차원**이고, `bake_manager` 의 기록 경로만 `self.axes` 로 거른다
  (`session.set_axes()` · `normalize_axes()` · `ROT_INDEX`). 빠진 축은 0 을 쓰는 게 아니라
  **아예 안 쓴다** — 레이어에 등록하지 않고(base 값이 보임), Bake Keys 는 `cutKey` 도 안 한다.
  - **★ 축을 바꾸면 프리뷰 레이어를 반드시 지워야 한다.** 살아 있는 레이어에는 이미 예전 축의
    커브가 들어 있어서, 축을 끈 뒤에도 그 커브가 계속 원본을 덮어써 **꺼진 것처럼 보이지 않는다**.
    `set_axes` 가 바뀜 때만 지운다(샘플 캐시는 축과 무관 → 재샘플링 없음). Apply 는
    **promote 판정 앜에** 축을 확정해야 예전 축의 프리뷰가 승격되지 않는다.
  - UI 는 **마지막 한 축을 둆 수 없게** 되돌린다 — "축 0개" 예외 경로 자체를 없앨다.
  - 결과의 뜻: "그 평면으로 시뮬레이션" 이 아니라 **3차원 해의 그 축 오일러 성분만** 쓴 것.
