...
v01.01 — IMPLEMENTED (headless-verified, Maya test + push pending): added **Reverse button** to the shared TSL widget and turned it ON in A00350. `Framework/qt/MOD_tsl_qt_v01.py` now takes `show_reverse=False` (default OFF, so the other ~18 TSL tools are unaffected) → renders a `Reverse` button under Sort; `_on_reverse()` does `_set_records(list(reversed(self._records())))` — record-based like Sort so each item's UUID (uuid, component) data travels with it. A00350 `main_window` TSL sets `show_reverse=True` to flip the array element order in one click. Headless PySide test: default→no btn, True→order reversed + UUID data preserved.
