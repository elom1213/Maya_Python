...
- ~~`docs/study/`~~ = 학습 노트는 2026-08-21 부로 **별도 저장소 `JUN_Study`** 로 이전됨.
  이 폴더엔 안내 README 만 남아 있다. → [[study-docs-number-prefix]]
...
방법론/기법 학습 노트=**`JUN_Study` 저장소**(이 repo 아님). 새 문서는 해당 폴더/저장소의
README 목록 표에 한 줄 등록한다. 경계는 "툴을 설명하면 Maya_Python, 원리를 공부하면 JUN_Study".
