...
숨김 판정 = **메시 자신(쉐입 + 트랜스폼)만**의 visibility/lodVisibility/디스플레이 레이어/드로잉 오버라이드, Orig 제외.
**사용자 결정(v01.04)**: "메시가 아닌 오브젝트는 hide 돼 있어도 좋다" → 조상은 안 본다(v01.03 은 모든 조상). 숨긴 그룹 안의 켜진 메시도 통과.
