...
class JUN_matcher_v02():
    ...
    def match(self):
        for i in range(0, self.num_iter):
            member_flw = self.flw[i]
            member_tgt = self.tgt[i]
...
def JUN_CR_match( cage_glo, tsl_tgt, tsl_flw):
    mcr = JUN_matcher_v02()
...
def PY_JUN_makeUI_ContrlRigTool ():
    ...
    cmds.window( str_winName, bgc=color_mainDark, title="Control Rig Tool V01" );
    ...
    cmds.button( "name_btn_CR_match", 
                 ...
                 label='Match', 
                 bgc=color_btn, 
                 command= lambda *argv : JUN_CR_match(JUN_cage_glo, "JUN_CT_tgts_tsl", 
                                                                    "JUN_CT_flw_tsl"));
