...
# get BlendFKIKPos_grp's 8 objects 
    ...
    if len(str_BlendFKIKPos_grp) ==0 :
        cmds.text(text_state, e=True, label= 'State : Fail to find objects for setup');
        return 0;
    ...
    if len(str_set_posAll) != 8:
        cmds.text(text_state, e=True, label= 'State : Fail. Number of objects is not 8');
        return 0;
...
# get 8 controlers
    ...
    if len(str_list_ctlsIK) != 8:
        cmds.text(text_state, e=True, label= 'State : Fail. Number of controlers is not 8');
        return 0;
...
# Update textScrollList : Targets, Followers
    ...
    cmds.text(text_state, e=True, label= 'State : Sucess');
