...
def get_meshShapes_from_obj(str_list_objs):
    str_list_objs_descendents = [cmds.listRelatives(str_obj, f = True, allDescendents = True) for str_obj in str_list_objs];
    return [i[0] for i in str_list_objs_descendents if cmds.nodeType(i) == "mesh"];
...
def cmd_upd_tsl_SGs (name_tsl_SGs):
    ...
    # for str_upStream in str_downStreams_fromObj:
    ...
    for str_dwnStream in str_list_dwnStrm_fromShapes:
        if(cmds.nodeType(str_dwnStream) == "shadingEngine" and 
           str_dwnStream not in str_list_memo):
            ...
            str_list_memo.append(str_dwnStream);
